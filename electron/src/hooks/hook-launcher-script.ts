export function buildLauncherScript(): string {
  return [
    ...
    '  if (source === "claude" || source === "codex" || source === "trae" || source === "traecli" || source === "traex") {',
    "    return source;",
    "  }",
    '  return "unknown";',
    "}",
    ...
    "}",
    "",
    "async function postAgentHook({ endpoint, token, terminalSessionId, agent, hookEvent, threadId, panelId, tmuxPaneId, operationId, commandName }) {",
    "  try {",
    ...
    "}",
    "",
    "async function postCompletionHook({ endpoint, token, terminalSessionId, payload, source, completionReason, rawEvent, operationId, commandName }) {",
    "  try {",
    "    const response = await fetch(endpoint, {",
    '      method: "POST",',
    "      headers: {",
    ...
    '  const stateHookEvent = (source === "codex" || source === "trae" || source === "traecli" || source === "traex") ? toAgentHookStateEvent(normalizedEvent) : null;',
    ...
    "  const operationId = process.env.RUNWEAVE_TERMINAL_AGENT_OPERATION_ID || null;",
    ...
    "    const result = await postAgentHook({ endpoint: stateEndpoint, token, terminalSessionId, agent: source, hookEvent: stateHookEvent, threadId, panelId: terminalPanelId, tmuxPaneId, operationId, commandName: args.commandName });",
