...
import { resolveEffectiveBetaPoolPaths } from "../dev-session/beta-pool/storage/paths.mjs";
import {
  ...
  for (const instanceId of await collectLegacyInstanceIds(
    ...
    instances.push({
      ...
      suggestedCleanupCommand: trusted
        ? `node scripts/beta/index.mjs legacy-cleanup --instance ${instanceId} --json`
        : null,
    });
  }
  return {
...
export async function cleanupLegacyBeta({
  ...
  const result = {
    ...
    journalPath,
    entries,
    restoreCommand: `node scripts/beta/index.mjs legacy-restore --operation ${operationId} --json`,
    purgeCommand: `node scripts/beta/index.mjs legacy-purge --operation ${operationId} --confirm ${operationId} --json`,
  };
