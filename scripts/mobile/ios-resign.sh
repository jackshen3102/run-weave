...
#
# 用法：
#   ./scripts/mobile/ios-resign.sh              # 自动选择已连接的设备
#   ./scripts/mobile/ios-resign.sh --check      # 只查描述文件剩余天数，不做任何改动
#   ./scripts/mobile/ios-resign.sh --device <UDID>   # 指定设备
#   ./scripts/mobile/ios-resign.sh --skip-web   # 跳过 web 构建（仅重签名，更快）
#
set -euo pipefail
...
log "下次到期前重新运行本脚本即可；查剩余天数: ./scripts/mobile/ios-resign.sh --check"
