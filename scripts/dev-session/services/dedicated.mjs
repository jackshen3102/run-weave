...
import { buildAgentTeamFixtureEnvironment } from "../fixtures/scope.mjs";
...
export async function startDedicatedBackend({
  ...
  const processInfo = spawnDetached({
    ...
    env: {
      ...
      RUNWEAVE_DESKTOP_STATE_DIR: path.join(paths.sessionDir,"electron","user-data"),
      RUNWEAVE_DESKTOP_CHANNEL: "stable",
...
export async function startDedicatedElectron({
  ...
  const electronEnv = {
    ...
    RUNWEAVE_WHISTLE_PORTS: whistlePorts.join(","),
