...
const cases = [
  ...
  },
  {
    ...
    run() {
      ...
      const directInvocation = spawnSync(
        process.execPath,
        ["./scripts/update/index.mjs", "--dry-run"],
        {
          cwd: process.cwd(),
          encoding: "utf8",
          env: directEnv,
  ...
  },
  {
    ...
    run() {
      assert.equal(isAppSensitivePath("scripts/update/index.mjs"), true);
      assert.equal(
        isAppSensitivePath("scripts/update/core.mjs"),
        true,
      );
      assert.equal(
        isAppSensitivePath("scripts/update/operations.mjs"),
        true,
      );
      assert.equal(
  ...
  },
  {
    ...
    run() {
      const plan = resolveUpdatePlan({
        changedFiles: ["scripts/update/index.mjs"],
        hasPreviousState: true,
        installedAppVersion: "0.124.0",
        sourceShellVersion: "0.124.0",
      });
      assert.equal(plan.mode, "app");
      assert.equal(plan.reason, APP_UPDATE_REASON_NATIVE_CHANGE);
      assert.deepEqual(plan.nativeFiles, ["scripts/update/index.mjs"]);
    },
  },
  {
