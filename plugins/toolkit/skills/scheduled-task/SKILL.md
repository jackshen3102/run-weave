...
3. 明确一次性时间的日期、时区和 UTC 转换；“明早”“工作日”等相对说法要按当前日期解释并给出绝对日期/规则。缺少项目、时间、时区或任务内容，或存在两种合理解释时先询问。任务提示词提到另一个 Skill 时，确认它在预期后台 Agent 环境可用；无法确认就告知用户这一限制。
   提示词要求真实 Browser、iOS 模拟器、命令联网、Git 元数据或工作区外本机访问时，建议用户选择
   `full-access`，并说明它无沙箱且不等待交互审批。具体能力和验收要求仍写入提示词并遵循对应 Skill；
   不增加或暗中生成 Browser、模拟器、网络、Git 等独立开关，也不因选择 full access 自动调用工具。
...
- 以验证响应的 `validation.config` 为准，向用户展示目标 Backend/profile、项目名称/路径/ID、任务 ID（编辑）、完整名称与**完整提示词**、provider/model/effort、执行权限、启用状态、时间规则与时区、下一次及后续预览的本地和 UTC 时间、补跑策略，以及编辑前后变化。连同 `sha256` 明确标识本次请求。`full-access` 草稿必须再次突出无沙箱、无交互审批及本机访问风险。预览不保证电脑届时在线；不要声称任务已经创建。
