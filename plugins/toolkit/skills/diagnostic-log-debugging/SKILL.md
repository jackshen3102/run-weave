...
本技能负责新增、录制和分析 AI 诊断日志，不负责制定“未复现就不修复”的决策门槛。用户显式要求先做真实复现、再决定是否修复时，使用 `$toolkit:reproduce-before-fix`；同时需要诊断日志时组合使用两个技能。只有沿真实产品入口采集的日志才能作为真实复现证据，模拟路径或伪造状态产生的日志不能替代复现。
...
3. 通过 UI 录制复现过程。
   在 Web 终端工作区右上角点击 `More actions`，选择 `日志上报`，点击 `开始记录`，执行能复现问题的用户操作，然后点击 `结束并上报`。优先使用 UI 控件，不要优先使用裸 HTTP 调用，这样才能捕获浏览器侧日志。
