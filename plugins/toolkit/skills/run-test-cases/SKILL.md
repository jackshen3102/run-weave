...
1. 读取完整 YAML，确认顶层只有 `version`、`name`、`description`、`cases`，每条 case 只有
   `id`、`name`、`required`、`description`、`preconditions`、`steps`。
2. 先运行项目格式校验。Runweave 仓库使用：
...
3. 校验失败立即停止，不猜测或修补格式。
4. 默认只执行 `required: true` 的 case，并按文件顺序逐条执行。只有用户明确要求全量回归时，
