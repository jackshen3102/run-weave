...
| 真机无线已安装 App，只需检查启动       | 使用辅助脚本 launch，直接经 CoreDevice 启动；不依赖 XCTest，不宣称 UI 验收通过                                   |
| runner code 74 / IDE 通道断开          | 检查本次 .diagnostic.json 和 runner.log；先区分 App 启动与 XCTest 握手，不靠延长超时、重复重签或重放业务动作恢复 |
