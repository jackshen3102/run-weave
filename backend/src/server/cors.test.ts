...
describe("createCorsMiddleware", () => {
  ...
  it("allows capacitor app origins without explicit configuration", async () => {
    const app = express();
    ...
    const response = await fetch(`http://127.0.0.1:${port}/api/auth/login`, {
      ...
      headers: {
        ...
        "Access-Control-Request-Method": "POST",
        "Access-Control-Request-Headers": "content-type,x-auth-client",
      },
    });
  ...
  it("allows ionic app origins without explicit configuration", async () => {
    const app = express();
    app.use(createCorsMiddleware([]));
    app.post("/api/auth/login", (_req, res) => {
      res.status(200).json({ ok: true });
