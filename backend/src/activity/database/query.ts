...
import { decryptActivityValue } from "../security/crypto";
...
export interface FactRow {
  ...
  result_status: ActivityFactDto["result"] extends { status: infer T }
    ? T
    : never;
...
function encodeCursor(occurredAt: number, activityOffset: number): string {
  return Buffer.from(
    JSON.stringify([occurredAt, activityOffset]),
    "utf8",
  ).toString("base64url");
}
...
export function activeDeleteTombstoneSql(): string {
  return `NOT EXISTS (
...
export function queryActivityTimeline(
  ...
  const summaryParts = buildQueryParts(
    { ...selectorQuery, cursor: undefined },
    asOf,
  );
  ...
  return {
    ...
    computed: {
      ...
      ((summary.operation_count === summary.event_count &&
        ...
        ? {
            durationMs:
              summary.last_occurred_at_ms - summary.first_occurred_at_ms,
          }
        : {}),
    },
    ...(hasMore && last
...
export function queryActivitySources(
  database: Database.Database,
): ActivitySourceDto[] {
  const rows = database
    .prepare(
  ...
  return rows.map((row) => ({
    ...
    gapRanges: (
      readGaps.all(row.producer_instance_id, row.producer_boot_id) as Array<{
    ...
    ...(row.last_commit_latency_ms != null
      ? { lastCommitLatencyMs: row.last_commit_latency_ms }
      : {}),
...
export function queryActivityDataPolicy(
  ...
  const pendingDeleteJobs = Number(
    (
      database
        .prepare(
          "SELECT COUNT(*) AS value FROM activity_delete_jobs WHERE status != 'completed'",
        )
        .get() as { value: number }
    ).value,
  );
  return {
    ...
    databaseBytes: fs.existsSync(databasePath)
