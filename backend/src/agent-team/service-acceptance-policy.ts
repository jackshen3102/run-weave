...
export function environmentRecoveryProbeForDispatch(
  ...
  const affectedCases = acceptanceCasesForRole(run, "behavior_verify").filter(
    (item) =>
...
export function resolveEnvironmentRecoveryIntervention(
  ...
  const behaviorCases = acceptanceCasesForRole(run, "behavior_verify");
  const blockers = behaviorCases.filter(isRunScopedEnvironmentBlocker);
...
function formatEnvironmentRecoveryOptions(
  ...
  return Array.from(firstCaseByFingerprint, ([fingerprint, caseId]) =>
    `${fingerprint}(${caseId})`,
  ).join(", ");
}
