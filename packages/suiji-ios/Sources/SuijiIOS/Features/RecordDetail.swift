...
struct RecordDetail: View {
  ...
  private var record: SuijiRecord {
    let candidates = [original, refreshed, session.records.first { $0.id == original.id }].compactMap { $0 }
  ...
  var body: some View {
    ScrollView {
      VStack(alignment: .leading, spacing: 20) {
        ...
        if let onReview { Button("聊聊这条") { onReview(record) } }
        if pending { Button("状态结果待确认 · 重试确认") { Task { await session.setStatus(record, target: .done) } } }
        else if record.taskStatus == .open {
          HStack { Button("完成") { Task { await session.setStatus(record, target: .done) } }; Spacer(); Button("不再做") { Task { await session.setStatus(record, target: .archived) } } }
        }
        if record.kind == .task, record.taskStatus != .open {
      ...
      .toolbar { Button("编辑") { Task { await session.openEditor(record: record) } }.disabled(pending || session.statusBusy.contains(record.id)) }
