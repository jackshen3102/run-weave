...
const buildRoot = resolve(repo, ".runweave/suiji/ios-build");
const project = resolve(pkg, "ios/Suiji.xcodeproj");
const lock = resolve(
  project,
...
try {
  ...
  if (command === "doctor") {
    ...
    const app = resolve(
      buildRoot,
      `DerivedData/Build/Products/${configuration}-iphonesimulator/Suiji.app`,
    );
    if (command === "build") {
      ...
      run("xcrun", ["simctl", "launch", simulator, "com.runweave.suiji"]);
    }
  }
} catch (error) {
