...
- Files/Changes 正文只读，支持确认后的单文件删除与 Reset，不支持目录和批量操作。Staged Reset 仅取消暂存；Working Reset 恢复到暂存区版本，未跟踪文件则删除。目录和搜索按项目隔离，相对越界及越界符号链接由后端拒绝；后端允许显式绝对路径的只读预览，因此 projectId 不是所有文件预览的沙箱承诺。
- 文件、Diff 和全屏图片使用同一系统导航栈，左边缘右滑返回由 iOS 管理；不叠加自定义拖拽、延时移除或导航栏显隐动画，避免与正文横滚及图片平移争抢手势。
- 文件内的“查看变更”和全屏图片进入下一级，返回保留原文件的滚动与加载状态；重新从列表打开则读取当前内容。返回转场期间保留目标数据，不提前置空；搜索键盘在打开预览时收起。
- SVG 使用非持久 WKWebView，关闭 JavaScript、禁止网络与导航，不带认证或终端 bridge。
- Diff 的 800 总行数 / 180,000 行数乘积预算限制计算成本，大输入降级为删除/新增并在后台计算。
- 预览缓存属于不可变 endpoint 的 APIClient：15 秒 fresh、30 分钟闲置回收、32 MiB LRU；登录、清凭据和切连接使旧请求失效，迟到结果不能回填。文件删除或 Reset 请求结束后使当前项目的预览缓存和在途读取失效；成功后刷新目录、搜索与变更摘要，关闭旧预览。
