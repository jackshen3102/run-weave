...
暂存、工作区及删除前图片版本遵循[统一预览合同](../../../docs/architecture/terminal-code-preview.md#编辑冲突与-git)。
...
点击后由独立的底部模态容器承载输入：正文从一行起步，按真实排版高度向上增长，达到顶部安全区后
...
按手动排序取前 3 条，在输入框上方单行置顶；横屏收为“全部”入口。点击置顶条目直接在光标处插入，
选择文字时替换选区，保留 UIKit 原生撤销行为、其余草稿和附件；“全部”直接打开回复库。
保存当前草稿使用工具栏的独立入口。正文只填入本地编辑器，仍需显式点击发送，
保持既有离线禁止发送、接受确认、输入不重试与 agent-aware 模式。回复库由输入面板呈现独立 Sheet，
不重挂底层终端；录音/转写忙时禁用进入，存储写入期间禁用关闭。发送前临时编辑不改写库原文。

回复编辑页的正文从紧凑高度随内容增长，保存入口固定在导航栏右上角。返回和下拉关闭均检查未保存内容，
有改动时可保存、放弃更改或继续编辑；未改动直接退出。由终端草稿带入的新回复也视为未保存内容，
保存失败保留编辑页与正文，放弃回复编辑不修改来源终端草稿。
