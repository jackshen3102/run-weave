...
iOS 15 使用普通系统 Sheet。终端内容忽略键盘安全区变化，包含回复库搜索键盘；键盘只改变
Sheet 内部布局，底层终端保持挂载和原始 bounds，
...
切换目标取消旧请求等待，迟到结果不能清理新目标草稿。留在原面板时，发送确认成功后关闭 Sheet，
...
保持既有离线禁止发送、接受确认、输入不重试与 agent-aware 模式。回复库在输入 Sheet 内导航，
