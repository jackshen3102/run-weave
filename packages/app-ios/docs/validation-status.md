...
2026-09-23 新增原生定时任务入口、列表/编辑/详情、运行输出、停止、历史终端恢复与来源回跳。
当前源码通过 iOS 26.5 Simulator Debug 构建、安装及实际交互；经用户授权借用空闲随记槽位，
保留其原 App 和数据，没有占用另一任务的 Runweave 槽位。

原生实际通过：首页更多入口、空列表、新建、搜索无结果/清空恢复、每日改为每周一/三及时间预览、
模型选择保存、暂停、删除确认、当前列表移除与归档历史保留、进程重启后的登录和任务读取。
真实 Codex 执行成功，手机查看完成状态/输出、恢复普通终端、重复打开和来源回跳均通过。
用同一隔离 Backend 的 API 模拟另一端修改，手机显示版本冲突并保留草稿，确认后加载最新配置；
编辑后的历史仍显示旧配置快照。停止 Backend 后显示缓存与离线提示，禁用新建。

独立 API 验证另覆盖四种安排预览、新建/立即运行幂等、历史快照不随编辑变化、停止最终进入 cancelled、
恢复 starting → ready 与重复打开复用终端。生产 Swift DTO 成功解码实际 Task/Run、能力、模型、
项目上下文、输出和恢复响应。API 通过不等于对应原生交互已逐项通过。

本机证据入口为 `.runweave/scheduled-tasks-qa/REPORT.md`，控件树与安装身份位于
`.runweave/mobile-qa/ios-scheduled-borrowed-20260923/`。最终安装输入摘要为
`80facdea8f5dae9a19e13b7c4fb91b88054c083083057c07db9ee2422af3d00b`。

仍未全量执行：分页/项目筛选、多种安排的原生保存矩阵、提交响应丢失、后台/快速连接切换隔离、
旧 Backend/503/认证失效、终端被改用后的恢复、Web 同时在线对照，以及大字体/iPad 布局及其他真机尺寸。
本轮是核心链路专项验收，不能标记完整计划全部通过；模拟器阶段未更新个人真机或正式 Backend；后续真机结果见下文。

本次隔离 Dev Session 已停止，测试终端/项目、本地测试连接与凭据已移除，借用槽位已归还；原随记 App 和其他任务租约保持。

同日 rebase 至 `e85f922b` 后，通过 iPhone 17 / iOS 26.6.1 真机 Debug 构建、签名、覆盖安装与实际交互。
最终构建 ID `20ca2c6c-ee52-4800-a9d0-83164716bc5d`，输入摘要
`ad00a1d3b5aa1a5e45384687b18bb1270fdb12d5169a48d3a5dfe3c683127a10`。经用户明确授权正常停止另一任务旧自动化会话；无线 XCTest
曾在握手阶段以 code 74 / IDE disconnection 退出，切换 USB 后控件树和操作成功，不将无线失败记为业务失败。

真机核心通过：隔离连接登录、首页更多入口、空列表、创建每日任务（中文名称、英文提示词）、
编辑为工作日、真实 Codex 完成、输出查看、第二次执行停止并由 Backend 确认 cancelled、暂停后续安排、
删除确认/当前列表移除/归档两条历史保留、进程重启后登录与历史终端读取。
恢复 ready 后进入普通终端，任务页来源回跳与重复打开复用绑定均已实测。

补验发现首页历史终端的来源回跳会短暂显示任务页后退回首页，已将 RootView 改为单一根导航入口，
直接替换根目的地，移除固定延时 pop/push。最终构建在同一真机重跑首页来源回跳、
任务页打开终端、来源返回与连接切换通过；切回原“家里mac”后显示该电脑现有任务，没有残留测试记录。
创建/停止等先前成功路径未在最后仅导航修复后完整重跑，不扩大为全矩阵通过。

第三方键盘展开时，自动化以 TextEditor 中心点击会命中键盘，提示词未录入；改点可见输入区域后
正文正确进入表单并实际保存。这是本次操作限制，没有据此宣称所有键盘/可访问性组合均已验证。
原始证据在 `.runweave/mobile-qa/ios-scheduled-phone-20260923/`；截图、API 后置记录与构建安装回执
见 `.runweave/scheduled-tasks-qa/PHONE-REPORT.md`。测试连接和凭据、专用终端/项目/目录已移除，
任务只在已停止隔离 profile 中归档；Dev Session `dvs-72d797` 和 relay 已停止，手机保留新版并恢复原连接。
正式 Backend 与原有任务未修改。本轮仍不代表上面列出的分页、异常时序与布局矩阵全部通过。

同日按用户收窄后的范围补验重点异常链路，不再纳入大字体、iPad 和其他尺寸。iPhone 17 真机实际通过：

- 25 条任务按 20 + 5 加载分页，项目筛选只显示该项目的两条记录。
- 新建与立即运行均在服务端成功后主动丢弃 HTTP 响应；手机显式重试使用相同幂等键及请求体，
  API 分别只存在一个 Task 和一个 Run。新建不确定时保留草稿、禁用普通保存并显示专用确认入口。
- 将旧连接列表响应延迟 45 秒后切换电脑，延迟结束仍显示新电脑任务；后台约 27 秒无任务轮询，
  后台期间另一端修改的任务在返回前台后自动刷新。
- 恢复历史后在其终端真实执行 `/new` 并完成另一段 Codex 对话；手机再次恢复时提示原终端被改用，
  确认后分配新终端并恢复原 thread，旧终端的另一段 thread 保留。

补验中一次嵌套终端来源点击后仍停留在已断开的终端页，原生返回键可恢复。针对这条路径，
来源点击现在先显式 dismiss 嵌套页面，再关闭控制器；首页的来源仍走原地替换。
最终构建 `60057022-a0e9-4dd0-84f5-709fd852992f`，输入摘要
`b0ddbf7cb8c06922fe6b2490b9392bddcbba7632297d0014705c1760c64160d1`，已签名覆盖安装。
真机回归首页来源、嵌套来源、嵌套终端改用新对话后来源均通过；没有用有限次通过声称穷尽所有时序。
本次四组重点测试来自前一构建，最终仅导航改动后重跑了上述导航路径。

本机证据见 `.runweave/scheduled-tasks-qa/PRIORITY-REPORT.md` 和相同真机任务目录。
