...
export function ConnectionPage({
  ...
  const handleAdd = async () => {
    ...
    if (endpointId) {
      onAdd(trimmedName,"",endpointId);setName("");setEndpointId("");setShowForm(false);return;
    }
    if (!trimmedUrl) {
      setError("请输入后端地址");
      return;
      ...
      <section className="animate-fade-rise relative w-full max-w-lg rounded-[2rem] border border-border/60 bg-card/82 p-7 shadow-[0_34px_120px_-72px_rgba(17,24,39,0.82)] backdrop-blur-xl sm:p-9">
        ...
        {showForm && (
          <div className="mt-6 space-y-4">
            ...
            {tunnels?.config.backendEndpoints.length ? <label className="block text-sm">访问方式<select className="mt-2 h-10 w-full rounded-lg border border-border bg-background px-3" value={endpointId} onChange={e=>setEndpointId(e.target.value)}><option value="">后端地址</option>{tunnels.config.backendEndpoints.map(endpoint=><option key={endpoint.id} value={endpoint.id}>{tunnels.config.hosts.find(h=>h.id===endpoint.hostId)?.name} · Backend {endpoint.remotePort}</option>)}</select></label> : null}
            {!endpointId && <>            <div className="space-y-2">
              <label
                className="text-xs uppercase tracking-[0.24em] text-muted-foreground/70"
                htmlFor="conn-url"
              >
                  ...
                  <>
                    ...
                    >
                      ...
                      <p className="mt-0.5 text-xs text-muted-foreground">
                        {conn.tunnelEndpointId ? "通过独立 SSH 通道访问" : conn.url || "内置本地后端未连接"}
                      </p>
