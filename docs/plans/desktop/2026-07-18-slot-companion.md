...
| Electron 托盘     | electron/src/desktop/tray.ts 使用静态菜单管理主窗口                                                  | 增加持久化复选项，负责创建或销毁 Companion                                             |
...
新增 electron/src/companion/preferences.ts：
...
- electron/src/desktop/tray.ts：增加“显示桌面宠物”复选项；关闭时销毁窗口，开启时重新创建。
...
| frontend/src/services/terminal/index.ts                               | 复用 Panel focus 与 Session acknowledgement 请求     |
| electron/src/desktop-companion-window.ts                              | 独立窗口、定位、尺寸和显示器事件                     |
| electron/src/companion/preferences.ts                                 | enabled 偏好读写                                     |
| electron/src/desktop/runtime-state.ts                                 | 保存 companionWindow                                 |
| electron/src/preload.ts                                               | 最小 Companion IPC surface                           |
| electron/src/desktop/tray.ts                                          | 持久化开关和窗口生命周期                             |
...
- electron/src/companion/preferences.ts
- electron/src/desktop/runtime-state.ts
- electron/src/desktop/tray.ts
