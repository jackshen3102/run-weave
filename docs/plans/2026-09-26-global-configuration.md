...
当前执行记录（2026-09-27 01:00）：统一 YAML 库、143 字段合同、CLI 管理、Backend/Desktop/App Server/独立服务和 Web/iOS 配置入口已进入实现与验收阶段，尚未发布到 Stable。iPhone 17 已更新并启动，原生页面交互仍待验收。以下记录区分代码、安装和真实交互，不据此宣称全部阶段完成。
...
| 原生 iOS       | 当前源码已构建并安装到独立租约模拟器；实际登录第二 Dev，页面修改语言为 `en-US` 并保存，YAML、Backend 和页面均确认保存/生效版本为 5；进程重开后连接、登录态及该值/版本保持                                                                                                                                                                                                                                                | 切换连接取消请求、真机页面业务验收；已安装并启动但 XCTest 未完成                               |
...
真机更新使用唯一可用的 iPhone 17 硬件 UDID `00008150-00195CC03E33401C`。仓库固定只读套件先构建 Debug `com.runweave.app.native`，`BuildIdentity.json` 记录源码 `10e43141`、buildId `cf426f2d-b7c4-4a31-a988-d98502647479`；CoreDevice 实际覆盖安装一次，安装路径从原 `7B78226A-…` 变为 `9C68862D-…`，再次查询仍为该新路径。固定 XCUITest runner 启动后 CoreDevice 远端通道失效，三条只读用例未执行；随后 agent-device 的仅启动入口返回 App PID 39809、`uiVerified: false`。证据在 `packages/app-ios/.build/ios/device/runs/f2b3552c-ad72-416e-bc46-a55130216d63/` 与 `.runweave/mobile-qa/config-merged-iphone17-launch/`。安装和启动不代替设置页业务验收，用户可在手机手测。

未完成门槛仍包括完整配置来源/默认合同复核、旧独立 Beta 入口退役、首次初始化与安装态完整备份恢复流程、跨环境/切换/升级失败矩阵、独立测试用户下 Stable 验证、真实整机重启及真机页面交互验收。本机 Docker daemon 不可用。模拟器测试连接与凭据已移除、原连接恢复、租约已释放；本机 HTTPS fixture 已停止且测试发布配置已清除。Stable 未更新；用户手机已安装并启动当前构建，不能据此宣称设置页面已通过。五份测试计划的 49 条 required 用例不得因局部证据被整体标记通过。
