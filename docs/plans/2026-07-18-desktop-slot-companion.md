...
6. Completion 必须先成功打开目标 Session，之后才确认精确 completionRevision；Panel 失效可降级到 Session，Connection 或 Session 跳转失败不得确认。
...
| Completion 确认  | 现有 Session acknowledgedCompletionRevision               | Session 成功打开后，按 intent 的精确 revision 写入 | 服务端持久化                        |
...
6. completionRevision 非空时，只有第 3 步成功后才调用现有 acknowledgement，写 intent 携带的精确 revision，不读取并确认更晚 revision。
7. 返回 opened 或 opened_with_panel_fallback；连接失败、Session 不存在或超时返回失败。

Companion 收到结果后：

- completed：成功结果才等待 snapshot 反映 acknowledgement；失败时保留提醒。
...
3. 只在 Session 成功后确认 intent 携带的 completionRevision。
...
- Completion 跳转失败后 acknowledgedCompletionRevision 不变。
...
| Renderer 先退役、实际跳转后失败                   | 用户丢失提醒                        | 用 main renderer 结果握手，成功后才退役                                |
...
- Completion、失败事实与 Human Gate 分别使用正确的退役语义。
