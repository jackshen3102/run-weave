...
- SSH 登录用户与 Backend 运行用户可以不同。实验环境分别为 `shenxiaojie.316` 和 `root`；通过既有 Backend 的业务权限访问已有项目，不自动按 SSH 用户另起第二套服务，也不把自动 sudo 作为桌面连接流程。
...
目标：`shenxiaojie.316@10.37.216.239`；用户明确允许作为实验环境，并指定本地 Browser 2，Browser 1 由其它 Agent 使用。
