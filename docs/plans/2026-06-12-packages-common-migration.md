...
推荐包名：`@browser-viewer/common`。用户提到的 `commom` 按拼写错误处理，落地目录使用 `packages/common`。

## Common 准入标准

代码进入 `packages/common` 必须满足下面至少一条：

1. 当前 App 和 Web 已经都有调用方，迁移后两个调用方都会从 `@browser-viewer/common` 引入。
...
- App 当前实际使用 `TerminalRenderer`：`app/src/pages/AppTerminalPage.tsx` 和 `app/src/hooks/use-app-terminal-connection.ts` 从 `@browser-viewer/terminal-renderer` 引入类型/组件。
- Web 当前没有使用 `TerminalRenderer` 组件；Web 只在 `frontend/src/index.css` import 了 `@browser-viewer/terminal-renderer/terminal-renderer.css`。
...
- `app/src/pages/AppTerminalPage.tsx` 从 `@browser-viewer/common/terminal` 引入 `fileToBase64`、`shellQuote`。
- `frontend/src/components/terminal/use-terminal-emulator.ts` 从 `@browser-viewer/common/terminal` 引入 `fileToBase64`、`shellQuote`、`isTerminalAutoResponse`、`isShiftEnterLineFeed`。
- `frontend/src/components/terminal-page.tsx` 从 `@browser-viewer/common/terminal` 引入 `isTerminalAutoResponse`、`isShiftEnterLineFeed`。
- `packages/terminal-renderer/src/TerminalRenderer.tsx` 从 `@browser-viewer/common/terminal` 引入 `isTerminalAutoResponse`、`isShiftEnterLineFeed`，从而让 App 的 renderer 路径也复用同一逻辑。

依赖要求：

- `app/package.json` 添加 `@browser-viewer/common: workspace:*`。
- `frontend/package.json` 添加 `@browser-viewer/common: workspace:*`。
- `packages/terminal-renderer/package.json` 添加 `@browser-viewer/common: workspace:*`。

验证：

- `pnpm --filter @browser-viewer/common typecheck`
...
当前 `terminal-renderer.css` 被 App 和 Web 都 import，但它挂在 `@browser-viewer/terminal-renderer` 包下。这里有两个可选方案，只选一个：

方案 A：暂不迁 CSS。

- 保持 `app/src/main.css` 和 `frontend/src/index.css` 继续 import `@browser-viewer/terminal-renderer/terminal-renderer.css`。
...
- `@browser-viewer/common` 导出 `./terminal-renderer.css`。
- App/Web CSS 入口都改为 `@browser-viewer/common/terminal-renderer.css`。
...
- `rg -n "terminal-renderer.css|@browser-viewer/terminal-renderer" app frontend packages package.json pnpm-lock.yaml`
- 如果选择方案 A，预期只有 App/Web CSS 和 renderer 包本身保留旧 CSS 引用。
- 如果选择方案 B，预期 App/Web CSS 都改到 `@browser-viewer/common/terminal-renderer.css`，且没有两份 CSS 内容。
...
- 只让它复用 `@browser-viewer/common/terminal` 的 key/output helpers。
...
- `@browser-viewer/common` 导出 `./terminal-renderer` 和 `./terminal-renderer.css`。
- App import 改为 `@browser-viewer/common/terminal-renderer`。
...
- 自动：`pnpm --filter @browser-viewer/common typecheck`、`pnpm --filter @runweave/app typecheck`、`pnpm --filter @browser-viewer/frontend typecheck`
...
- `pnpm --filter @browser-viewer/common typecheck`、`pnpm --filter @browser-viewer/terminal-renderer typecheck`、`pnpm --filter @runweave/app typecheck`、`pnpm --filter @browser-viewer/frontend typecheck` 通过。
