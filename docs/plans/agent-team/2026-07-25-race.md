...
- 现状：`docs/plans/terminal/2026-07-18-worktree-context.md:51` 明确「不在 Runweave 内执行 `git worktree add/remove/prune/repair`」；`WorktreeProjectRegistry` 只读 `git worktree list`。
...
- **实现前必须先更新** `docs/plans/terminal/2026-07-18-worktree-context.md`，把「不执行 worktree add」的约束修订为「除 Race 受控 `race-` 命名空间外，不执行」，让越界被记录、被批准。
...
### 5.2 后端 HTTP 合约（新增 `backend/src/routes/race.ts`）
...
- **新建** `backend/src/routes/race.ts`：5.2 的路由，挂到现有 route 注册处。
...
- **修改** `docs/plans/terminal/2026-07-18-worktree-context.md`：按 4.1 修订 worktree 写操作约束。
