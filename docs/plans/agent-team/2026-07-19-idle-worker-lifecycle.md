...
当前 [agent-hook-processor](../../../backend/src/terminal/application/agent-hook-processor.ts) 只允许 `UserPromptSubmit` 用匹配的 last-thread identity 恢复可信身份；`Stop` 不享有同一身份恢复规则。于是结束事件虽然有完整身份，仍会在 current identity 已清理时被当成 inactive。
...
修改 `backend/src/terminal/application/agent-hook-processor.ts`：
...
- `scripts/verify/agent-team/review-checkpoints/bootstrap/core.mjs`
...
- `backend/src/terminal/application/agent-hook-processor.ts`：唯一的 direct hook/completion 状态与 thread identity 判定权威。
- `backend/src/app-server/handlers/agent-lifecycle.ts`：lifecycle observation 的 Pane 级 identity 选择与补偿入口。
- `scripts/verify/agent-team/review-checkpoints/bootstrap/core.mjs`：生产 processor 与 Agent Team bounce 回归。
