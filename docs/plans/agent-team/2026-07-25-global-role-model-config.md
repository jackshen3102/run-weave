...
- 新增 `backend/src/agent-team/runtime/model-config-store.ts`
...
- `backend/src/agent-team/runtime/model-runtime.ts`
...
- `packages/shared/src/agent-team/index.ts`：导出 model config 合约。
- `backend/src/routes/agent-team/index.ts`：
...
修改 `frontend/src/features/terminal/state/workspace-store.ts`：
