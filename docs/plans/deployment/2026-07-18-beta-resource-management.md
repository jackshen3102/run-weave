...
- `scripts/dev-session/services/index.mjs`
...
```json
{
  "dev:pool": "node ./scripts/dev-session/commands/pool.mjs"
}
```
...
- `scripts/dev-session/beta-pool/core.mjs`
- `scripts/dev-session/beta-pool/storage/index.mjs`
- `scripts/dev-session/beta-pool/index.mjs`
- `scripts/dev-session/services/index.mjs`（仅在现有 inspection 不能区分 owned/shared facts 时补充只读返回值）
- `package.json`
...
- `scripts/dev-session/verify/index.mjs`
