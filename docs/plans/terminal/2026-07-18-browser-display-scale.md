...
> 配套测试计划：`docs/testing/terminal/browser/display-scale.testplan.yaml`
...
`packages/shared/src/browser/display-scale.ts` 是档位与校验事实源；`electron/src/browser/device/display-scale.ts` 是运行时应用事实源。
...
- `electron/src/browser/cdp/proxy/session-messages.ts`
- `electron/src/browser/view/index.ts`
...
```bash
pnpm testplan:validate docs/testing/terminal/browser/display-scale.testplan.yaml
```
