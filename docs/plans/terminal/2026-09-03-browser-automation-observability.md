...
> 代码基线：`main@383be140`
> 配套测试计划：`docs/testing/terminal/browser/automation-observability.testplan.yaml`
...
- `electron/src/browser/cdp/proxy/types.ts`
- `electron/src/browser/cdp/proxy/session.ts`
- `electron/src/browser/cdp/activity.ts`
- `electron/src/browser/runtime.ts`
- `electron/src/browser/workspace/index.ts`
- `electron/src/browser/annotation/index.ts`
...
扩展 `packages/shared/src/browser/profile.ts`，保持旧调用兼容：
...
新增 `packages/shared/src/browser/automation.ts` 并在 package exports 暴露明确子路径：
...
- 新建 `electron/src/browser/automation/capture.ts`：最小 frame producer 接口与完整 cleanup。
...
- [x] 新建 `packages/shared/src/browser/automation.ts`，定义 actor、connection、target、action、
...
- [x] 扩展 `packages/shared/src/browser/profile.ts` 的 resolver request/response 和 conflict
...
- [x] 新建 `electron/src/browser/automation/runtime.ts`，以 BrowserWindow 为 snapshot/订阅边界，
...
- [x] 在 `electron/src/browser/cdp/proxy/session.ts` command relay 成功进入目标 session 时更新
...
- [x] 在 `electron/src/browser/automation/runtime.ts` 内注册 get snapshot、set view state 和事件
...
```bash
pnpm testplan:validate docs/testing/terminal/browser/automation-observability.testplan.yaml
pnpm --filter @runweave/shared typecheck
pnpm --filter @runweave/cli typecheck
pnpm --filter @runweave/cli lint
pnpm cli:build
...
```text
docs/testing/terminal/browser/automation-observability.testplan.yaml
```
