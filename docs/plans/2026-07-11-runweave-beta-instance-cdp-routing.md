...
> 上层依赖：本计划是 `docs/plans/2026-07-11-runweave-dev-session.md` 的 `beta` profile 子计划。Planner、manifest、service identity、ownership、status/open/stop 和 CDP resolver 以 Dev Session 合约为准。Beta App identity、userData 和 CDP 必须实例独占；Backend、App Server 是否独占按代码、契约、状态和生命周期影响决定，未受影响时允许 `shared-declared`。
...
2. App 路径、userData、更新状态、Desktop CDP 端口和 status 文件都以单个 Beta 为前提；Backend profile 与 App Server home 也缺少由 Dev Session 明确表达 shared/dedicated ownership 的能力。
...
- 不让两个实例共享 Beta 自身的 userData、运行时指针、更新状态或 CDP Proxy；Backend profile 与 App Server home 是否共享由上层 Dev Session 影响闭包决定。
...
每个实例至少独占 Beta 自身资源；Backend 与 App Server 资源按 ownership 决定：
...
| backend profile      | dedicated 时 `<userData>/browser-profile`；shared-declared 时引用已握手 namespace     |
| runtime/update state | `<userData>/runtime`、`<userData>/update`                                             |
| App Server home      | dedicated 时 `~/.runweave/app-server-beta/<instanceId>`；shared-declared 时只引用实例 |
...
Agent 和命令始终运行在 Stable 主应用 terminal，Beta 只是被测目标。触发入口放在当前源码仓库，而不是依赖 Beta terminal、Beta shell env 或可能较旧的 Stable `rw` CLI：
...
resolver 必须忽略 Stable terminal 当前继承的 CDP endpoint 作为目标选择依据。Beta Electron 将两类 endpoint 写入实例状态，Stable 侧命令读取并验证；整个流程不要求在 Beta 中创建或进入 terminal。
...
验收：两个实例拥有的所有 Beta 可写路径不同；dedicated Backend/App Server 路径也不同，shared-declared 只记录引用且不归 Beta 所有；非法、空或超长 ID 在产生文件前失败；registry 使用原子写和 `0600` 权限。
...
- `electron/src/main.ts`：在 `requestSingleInstanceLock()` 前解析实例，设置独立 userData 和状态路径；Backend profile、App Server home 根据 manifest ownership 注入 dedicated 路径或显式 shared 连接。
...
- 需要 dedicated 的多 App Server 并行会增加端口、进程和磁盘占用；未受影响时优先 shared-declared。
...
- 两个 Agent 可同时用显式实例名验证不同 Beta revision；Beta-owned 可写状态互斥，shared-declared 服务的状态边界和共享原因可查询。
