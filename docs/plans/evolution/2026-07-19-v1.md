...
新增 `packages/shared/src/evolution/index.ts`，并仅通过 `@runweave/shared/evolution` 子路径导出。这里放跨 Backend/Web/CLI 的 DTO 和枚举，不放 SQLite row、Provider 私有参数或前端组件。
...
- `backend/src/activity/database/worker-protocol.ts`
- `backend/src/activity/recording/store.ts`
- `backend/src/activity/database/connection.ts`
- `backend/src/activity/database/query.ts`
- `backend/src/activity/database/service.ts`
