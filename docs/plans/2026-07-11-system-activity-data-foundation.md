...
interface ActivityEventInput<EventName extends string, Payload> {
  ...
  contentLinks: Array<{ role: ActivityContentRef["role"]; contentId: string }>;
  externalRefLinks: Array<{ role: ExternalRefLink["role"]; refId: string }>;
}

interface ActivityContentInput {
  contentId: string; // UUIDv7，由 SDK 首次组 batch 时生成，retry 不变
  mediaType: string;
  bytes: Uint8Array; // Producer 先做 schema redaction；Hub 仍会复核
}

interface ExternalRefInput {
  refId: string; // UUIDv7，由 SDK 首次组 batch 时生成，retry 不变
  authority: ExternalRef["authority"];
  locator: string;
...
interface ActivityIngestBatch {
  events: AnyActivityEventInput[];
  contents: ActivityContentInput[];
  externalRefs: ExternalRefInput[];
}

type AnyActivityEventInput = {
  [K in keyof P0PayloadRegistry]: ActivityEventInput<K, P0PayloadRegistry[K]>;
}[keyof P0PayloadRegistry];

type BehaviorFact<EventName extends string, Payload> = Omit<
  ActivityEventInput<EventName, Payload>,
  "scope" | "contentLinks" | "externalRefLinks"
> & {
  ingestedAt: string; // Hub transaction 的 ingest timestamp；COMMIT 前采样
  hubOffset: string; // Hub 分配的单调 offset，只表示接收顺序
  ingestFingerprintSha256: string; // 规范化 Producer input 的 Hub 幂等指纹
  scope: ActivityScopeInput & {
  ...
  retentionAnchorAt: string; // min(occurredAt, ingestedAt)，Hub 计算
  expiresAt: string; // anchor + schema/Project TTL
...
`POST /v1/events:batch` 一次携带同一 Producer identity 的 Event、Content part 与 External Ref descriptor。SDK 在首次组 batch 时生成稳定 `contentId/refId`，spool 与 retry 复用；Hub 从 capability token 注入 `owner_namespace`，禁止跨 namespace 引用。Hub 在一个 SQLite transaction 中验证、加密、写 row/link 后才 ACK，不采用“先上传内容、再写 Fact”的两阶段协议。同一 Content 被多个 Fact 引用时，`retention_anchor_ms/expires_at_ms` 只能缩短为所有链接中的最早值，后来的链接不能延长正文寿命。

HTTP/IPC 收到的 wire bytes 在反序列化边界当然是 `unknown`，但必须先经过 Registry discriminated-union parse，成功后才能成为 `ActivityIngestBatch`；业务代码中不存在 `ActivityEventInput<string, unknown>` 逃生口。
...
| `ingestedAt` / `hubOffset`                             | 是    | Hub               | Hub transaction 在 COMMIT 前采样 ingest time，并分配 64-bit 单调 offset                                      | Producer 输入中出现即拒绝             |
| `ingestFingerprintSha256`                              | 是    | Hub               | 对 schema-normalized Producer input、Content/Ref descriptors 做 canonical JSON 后计算 SHA-256；不含 Hub 字段 | 同 eventId/sequence 重试必须一致      |
...
| `producer.bootStartedAt`                               | 是    | SDK               | 生成 bootId 时读取并随 spool identity 持久化；该 boot 的所有 retry/event 保持不变                            | 合法时间且不晚于 occurred/ingested    |
...
| `runtime.appVersion/sourceRevision/backendProfileId`   | 否    | SDK               | Runtime manifest 明确暴露时写入；当前 backend 只有敏感目录路径，新增稳定 profile ID 前必须为空               | 格式、长度、secret scan               |
...
| Producer `contentLinks/externalRefLinks`               | 是    | SDK/Producer      | SDK 生成稳定 contentId/refId；Producer 只在同一 batch 选择 schema 允许的 role                                | ID、role、同 batch/既有对象 ownership |
| Canonical `contentRefs/externalRefs`                   | 是    | Hub               | Hub 从同一事务写入/验证的 Content、Ref 与 link rows 生成 digest、size、expiry、availability 快照             | 不接受 Producer 伪造快照              |
...
| `retentionAnchorAt` / `expiresAt`                      | 是    | Hub/Registry      | Hub 取 `min(occurredAt, ingestedAt)` 为锚点，再应用 schema TTL 与更短 Project policy                         | Producer 输入中出现即拒绝             |
...
CREATE TABLE behavior_facts (
  ...
  CHECK (producer_boot_started_at_ms >= 0 AND producer_boot_started_at_ms <= occurred_at_ms),
  CHECK (producer_sequence >= 1),
  CHECK (length(ingest_fingerprint_sha256) = 64),
  CHECK (retention_anchor_ms <= occurred_at_ms AND retention_anchor_ms <= ingested_at_ms),
...
CREATE INDEX facts_project_delete_idx
  ON behavior_facts (project_id, expires_at_ms, hub_offset)
  WHERE project_id IS NOT NULL;
CREATE INDEX facts_event_idx
  ON behavior_facts (event_name, occurred_at_ms, hub_offset);
CREATE INDEX facts_expiry_idx
  ON behavior_facts (expires_at_ms, hub_offset);
```

`payload_json` 不是 `unknown` 逃生口：写入前已经按 `eventName + schemaVersion` 验证。需要索引的 payload 字段由 Registry migration 建立带 event 条件的 `json_extract(...)` expression index，不引入通用 EAV 表。API 把 64-bit `hub_offset` 序列化为十进制字符串，避免 JavaScript number 精度丢失。

`ingest_fingerprint_sha256` 解决“唯一键命中但内容不同”的问题：相同 `eventId` 或相同 producer epoch/sequence 只有在 fingerprint 相同才返回原 ACK；fingerprint 不同必须以 `idempotency_conflict` 拒绝并写脱敏 quarantine 记录，不能静默覆盖或把冲突请求当成功重试。
...
interface ActivityContent {
  contentId: string;
  sha256: string;
  kind: "query" | "assistant_message" | "command" | "excerpt";
  mediaType: string;
  byteLength: number;
...
Query、可见 Assistant 回复、命令文本等核心内容都加密后写入 `activity.sqlite.activity_contents` 的 BLOB 列，Fact 只保存 ref 与 digest/size 快照，不 inline 正文到 `payload_json`。Content 与 Fact 都以 `min(occurredAt, ingestedAt)` 为留存锚点，spool/replay 不能重新开始 7/30 天倒计时。Content 到期时在事务中删除 ciphertext，保留 Fact link 的 digest/size 快照至 Fact 到期。

Registry 固定 Content role → kind：`query → query`、`response → assistant_message`、`command → command`、`tool_args/tool_result/excerpt → excerpt`。同一 `contentId` 只能用于兼容 kind/mediaType/owner 的 link；任何不兼容复用都以 `content_identity_conflict` 拒绝，不能因 hash 相同而跨语义合并。
...
CREATE TABLE activity_contents (
  ...
  CHECK (created_at_ms >= 0 AND retention_anchor_ms >= 0),
  CHECK (expires_at_ms >= retention_anchor_ms),
  CHECK (deleted_at_ms IS NULL OR deleted_at_ms >= created_at_ms),
  CHECK (current_availability IN ('available', 'expired', 'deleted')),
  CHECK (
    (current_availability = 'available' AND deleted_at_ms IS NULL AND ciphertext IS NOT NULL AND nonce IS NOT NULL AND auth_tag IS NOT NULL)
    OR
    (current_availability != 'available' AND deleted_at_ms IS NOT NULL AND ciphertext IS NULL AND nonce IS NULL AND auth_tag IS NULL)
  )
) STRICT;
...
CREATE TABLE fact_content_links (
  ...
  CHECK (ordinal >= 0),
  CHECK (byte_length_snapshot >= 0),
  CHECK (expected_expires_at_ms >= 0),
...
CREATE TABLE external_refs (
  ...
  CHECK (expected_expires_at_ms IS NULL OR expected_expires_at_ms >= 0),
  CHECK (last_checked_at_ms IS NULL OR last_checked_at_ms >= captured_at_ms),
...
CREATE TABLE producer_instances (
  ...
  highest_contiguous_accounted_sequence INTEGER NOT NULL,
  CHECK (boot_started_at_ms >= 0 AND first_seen_at_ms >= boot_started_at_ms),
  CHECK (last_seen_at_ms >= first_seen_at_ms),
  CHECK (highest_seen_sequence >= 0),
  CHECK (highest_contiguous_accounted_sequence BETWEEN 0 AND highest_seen_sequence),
  PRIMARY KEY (producer_instance_id, producer_boot_id),
  FOREIGN KEY (started_event_id) REFERENCES behavior_facts(event_id)
    ON DELETE SET NULL
    DEFERRABLE INITIALLY DEFERRED
) STRICT;
...
CREATE TABLE source_gaps (
  ...
  terminal_ingest_fingerprint_sha256 TEXT,
  retention_anchor_ms INTEGER NOT NULL,
  expires_at_ms INTEGER NOT NULL,
  CHECK (first_sequence >= 1 AND last_sequence >= first_sequence),
  CHECK (detected_at_ms >= 0),
  CHECK (closed_at_ms IS NULL OR closed_at_ms >= detected_at_ms),
  CHECK (retention_anchor_ms = detected_at_ms AND expires_at_ms >= retention_anchor_ms),
  CHECK (terminal_event_id IS NULL OR source_event_id IS NULL),
  CHECK (
    (terminal_event_id IS NULL AND terminal_ingest_fingerprint_sha256 IS NULL)
    OR
    (
      terminal_event_id IS NOT NULL
      AND length(terminal_ingest_fingerprint_sha256) = 64
...
CREATE TABLE ingest_quarantine (
  ...
  CHECK (encryption_key_version IS NULL OR encryption_key_version >= 1),
  CHECK (
    (ciphertext IS NULL AND nonce IS NULL AND auth_tag IS NULL AND encryption_key_id IS NULL AND encryption_key_version IS NULL)
    OR
    (ciphertext IS NOT NULL AND nonce IS NOT NULL AND auth_tag IS NOT NULL AND encryption_key_id IS NOT NULL AND encryption_key_version IS NOT NULL)
  )
) STRICT;

CREATE INDEX quarantine_expiry_idx
  ON ingest_quarantine (expires_at_ms, quarantine_id);
...
CREATE TABLE retention_sweeps (
  ...
  CHECK (last_started_at_ms IS NULL OR last_started_at_ms >= 0),
  CHECK (
    last_completed_at_ms IS NULL
    OR (last_started_at_ms IS NOT NULL AND last_completed_at_ms >= last_started_at_ms)
  )
) STRICT;
```
...
- `producer_instances` 可由任一合法 Event envelope 的 durable `bootStartedAt` 首次创建；`source.events_dropped` 还可用 payload 中从原 bundle 保存的 affected name/version/runtime/boot manifest 创建已丢失 epoch。`started_event_id=NULL` 明确表示启动 Fact 未到达或已过期，不阻塞后续真实事件，也不借 Reporter 当前版本伪造旧 epoch。
...
- 未知 sequence gap 的 `reason_code/source_event_id/terminal_event_id/terminal_ingest_fingerprint_sha256` 必须为 `NULL`。Producer 主动丢弃只有 `source.events_dropped` 能给原因并链接 `source_event_id`；Hub 对已收到但超过 retention 的请求写 `reason_code=retention_expired_before_ingest` 与 terminal event/fingerprint，此时 `source_event_id` 仍为 `NULL`。
- Foreign key 只证明 ID 存在；Hub transaction validator 还必须证明 `started_event_id` 指向同一 producer epoch 的 `producer.instance.started`，`source_event_id` 指向相同 producer/range 的 `source.events_dropped`，terminal receipt 的 eventId/sequence 未在 Fact 或其它 receipt 使用。语义不匹配按 schema reject，不能靠 FK 放行。
...
| 表                        | 字段                                                                                                   | 来源类别          | 原始来源与生成规则                                                                                          |
| ------------------------- | ------------------------------------------------------------------------------------------------------ | ----------------- | ----------------------------------------------------------------------------------------------------------- |
| `activity_contents`       | `content_id`                                                                                           | SDK               | SDK 首次组 batch 生成 UUIDv7；spool/retry 原样复用，Hub 校验 ownership                                      |
| 同上                      | `owner_namespace`                                                                                      | Hub               | 从已认证 capability token 注入；Producer body 不可覆盖                                                      |
...
| 同上                      | `compression/encryption`                                                                               | Registry          | 当前 schema 固定 `none` 或 `gzip`，加密固定 `aes-256-gcm`；Producer 不可覆盖                                |
| 同上                      | `encryption_key_id/encryption_key_version`                                                             | Hub               | Hub 从 Keychain key registry 选取当前 key identity/version，支持轮换且不把 key 写进数据库                   |
| 同上                      | `ciphertext/nonce/auth_tag`                                                                            | Hub               | Hub 使用对应 Keychain key 对允许落盘的 bytes 执行 AES-256-GCM 后生成                                        |
| 同上                      | `created_at_ms/retention_anchor_ms/expires_at_ms`                                                      | Hub/Registry      | Hub 记录 ingest time；anchor 取 Fact 的 `min(occurredAt, ingestedAt)`；按 7 天或更短 policy 算到期          |
| 同上                      | `redaction_version`                                                                                    | Hub               | 本次实际执行的最终 DLP/redactor 版本                                                                        |
| 同上                      | `current_availability/deleted_at_ms`                                                                   | Hub               | 初始为 `available/NULL`；retention/delete 事务清空密文时写 `expired` 或 `deleted` 与时间                    |
...
| `external_refs`           | `ref_id`                                                                                               | SDK               | SDK 首次组 batch 生成 UUIDv7；spool/retry 原样复用，Hub 校验 ownership                                      |
| 同上                      | `owner_namespace`                                                                                      | Hub               | 从已认证 capability token 注入；跨 namespace locator/ref link 拒绝                                          |
...
| 同上                      | `encryption_key_id/encryption_key_version`                                                             | Hub               | Hub 选用的 Keychain key identity/version；只存引用，不存 key bytes                                          |
| 同上                      | `version_or_digest`                                                                                    | Producer/Resolver | 源系统明确提供的 immutable version、revision 或 canonical object digest                                     |
| 同上                      | `captured_at_ms`                                                                                       | Producer/Resolver | source resolver 成功确认对象与版本时的结构化时间；Hub 校验 clock skew                                       |
...
| 同上                      | `availability_at_capture`                                                                              | Hub/Resolver      | Fact transaction 当时 resolver 返回的 `available` 或 `missing` 快照；以后不改                               |
| `producer_instances`      | `producer_instance_id/producer_boot_id/producer_name/producer_version/runtime_channel/runtime_surface` | SDK               | 已验证 Event envelope，或 loss payload 从原 spool bundle 保存的 affected manifest；Hub 不从当前 Reporter 猜 |
| 同上                      | `boot_started_at_ms`                                                                                   | SDK               | boot identity 的 durable start time；来自任一 Event envelope 或 loss payload，不依赖 started Fact           |
| 同上                      | `started_event_id`                                                                                     | Hub               | 同一 epoch 的 `producer.instance.started` Fact ID；该 Fact 未到达/已过期时为 `NULL`                         |
| 同上                      | `first_seen_at_ms/last_seen_at_ms`                                                                     | Hub               | Hub 首次/最近一次成功 COMMIT 该 epoch 的 ingest time                                                        |
| 同上                      | `highest_seen_sequence/highest_contiguous_accounted_sequence`                                          | Hub               | 根据已提交 Fact、已验证 loss range 与 terminal receipt 更新；不是 Producer 自报水位                         |
...
| 同上                      | `reason_code/source_event_id`                                                                          | Hub               | 已验证 `source.events_dropped` 可链接 event；Hub 也可写确定的 `retention_expired_before_ingest`，否则为空   |
| 同上                      | `terminal_event_id/terminal_ingest_fingerprint_sha256`                                                 | Hub               | 仅已过 30 天的单 sequence terminal receipt 写入；ACK 丢失重放时做幂等/冲突校验                              |
| 同上                      | `retention_anchor_ms/expires_at_ms`                                                                    | Hub/Registry      | 以 gap/receipt detected time 为锚点保留 30 天；独立 expiry index 清理，不被同 boot 新 Fact 延长             |
...
| 同上                      | `ciphertext/nonce/auth_tag/encryption_key_id/encryption_key_version`                                   | Hub               | 仅在大小与 secret scan 通过后加密；否则密文和 key identity/version 列均为 `NULL`                            |
| `retention_sweeps`        | `data_class`                                                                                           | Registry          | 固定的 retention class 名称，不接受任意 Producer 值                                                         |
| 同上                      | `last_started_at_ms/last_completed_at_ms/last_deleted_count/last_error_code`                           | Hub               | 每次有界 sweep 的执行状态；只作 telemetry，不能当成跳过迟到 row 的高水位                                    |
...
Registry 构建门禁必须从 `P0PayloadRegistry` 展开所有 leaf field，并校验每个 `eventName + schemaVersion + JSON Pointer` 恰好对应一个 `ActivityFieldSpec`；缺失、重复或来源为 generic unknown 都阻止 schema 发布。SQLite migration 也必须逐列出现在公共/辅助表 provenance 清单中，避免文档与实际列漂移。
...
| Agent Team   | `agent_team.worker.dispatched/result_recorded`          | Prompt 成功投递及通过 identity/freshness 校验的 outbox                             | run、worker role、panel、dispatch attempt、显式 finishedAt/result | pane/outbox                    | 识别角色承担的工作及其真实上报结果     |
...
| Collector    | `source.events_dropped`                                 | Producer 明确执行 quota/sampling 或 7 天 spool expiry，并先写 loss ledger          | producer、boot、sequence range、dropped count、reason code        | 无                             | 暴露已知丢弃；未知 gap 不伪造原因      |
...
`actor` 不是 Producer 可自由填写的标签；Registry 按 event family、producer capability 与结构化 reason/initiator 校验。拿不到 initiating actor 时必须写 `unknown`，不能按内容或时间猜。

| 事件族                                             | `actor.type/agent`                              | 确定依据                                                                                                       |
| -------------------------------------------------- | ----------------------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| `producer.instance.started`                        | `system`                                        | Registry 固定；表示来源进程/adapter 自身启动                                                                   |
| `terminal.session.created/deleted`                 | `unknown`                                       | 当前 API 没有可信调用者 identity；未来只有显式认证入口才能改成 user/agent                                      |
| `terminal.command.*`                               | 默认 `unknown`                                  | Shell marker 只证明命令发生；仅受控上游显式传播 actor identity 时沿用                                          |
| `user.query.submit_requested`                      | `user`                                          | `UserPromptSubmit` 的 prompt 是用户请求；必须来自受认证 Agent Hook capability                                  |
| `agent.thread.* / agent.response.* / agent.tool.*` | `agent` + 受认证 adapter identity               | Codex Hook 写 `codex`；其它 Agent 只能写自身 allowlist identity                                                |
| `browser.tab.* / browser.navigation.*`             | `user`、`agent`、`system` 或 `unknown`          | UI reason→user；带可信 Agent capability 的 CDP initiator→agent；page/restore/runtime→system；缺 marker→unknown |
| `verification.*`                                   | `agent/playwright` 或 allowlist runner identity | 独占 verification runner capability；普通 Agent/worker 不能写                                                  |
| `agent_team.*`                                     | `system`                                        | Backend orchestrator 在 durable transition/recording 边界写，不把 Worker 自报当 actor                          |
| `source.events_dropped`                            | `system`                                        | Producer SDK 的 durable loss ledger reporter                                                                   |
...
type P0PayloadRegistry = {
  ...
  "source.events_dropped": {
    ...
    affectedProducerVersion: string;
    affectedInstanceId: string;
    affectedBootId: string;
    affectedBootStartedAt: string;
    affectedRuntimeChannel: "stable" | "beta" | "dev" | "external";
    affectedRuntimeSurface:
      | "backend"
      | "app_server"
      | "desktop"
      | "web"
    ...
    reasonCode:
      ...
      | "retention_expired";
    priorityClass: "critical" | "normal" | "sampled";
  };
};
```
...
| `agent.tool.*`                          | `scope.threadId`                                  | 是   | Hook 公共 `session_id`                                                       | 需原样转发；不从 turn 文本反查            |
| 同上                                    | `scope.interactionId`                             | 否   | `(threadId, turnId)` 的 durable 精确映射                                     | 缺失则 unlinked，不按时间补               |
| 同上                                    | `payload.input/response.sha256/byteLength`        | 是   | 对 canonical、脱敏后 JSON bytes 计算                                         | Hub 重算并校验                            |
| 同上                                    | `payload.input/response.redactionCount/truncated` | 是   | 确定性 DLP 的替换计数与 schema byte limit 裁剪结果                           | 不由模型或 UI 填写                        |
| `user.query.submit_requested / agent.*` | `externalRefs[role=thread]`                       | 否   | 仅当注册 resolver 可用 `session_id` 定位稳定 Thread 版本时同 batch 注册      | 禁止使用不稳定 `transcript_path`          |
| `agent.tool.completed`                  | `externalRefs[role=artifact]`                     | 否   | 仅当结构化 `tool_response` 明确给出注册 authority、version/digest 与 locator | 任意文本路径不能自动成为 artifact Ref     |
...
| 事件                                   | 字段                                                                         | 必填 | 原始来源                                                                  | 当前状态                                                                               |
| -------------------------------------- | ---------------------------------------------------------------------------- | ---- | ------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- |
| `browser.tab.* / browser.navigation.*` | `scope.browserGroupId/tabId`                                                 | 是   | Electron `TerminalBrowserEntry.browserGroupId` 与调用方 tab ID            | 已有原值；需接 Activity emitter                                                        |
...
| `verification.completed`               | `payload.attempt`                                                            | 是   | 与对应 started 相同的 attempt                                             | 需新增；Hub 校验 operation/case/target 一致                                            |
...
| 事件                                | 字段                                                                                                 | 必填  | 原始来源                                                                        | 当前状态                                         |
| ----------------------------------- | ---------------------------------------------------------------------------------------------------- | ----- | ------------------------------------------------------------------------------- | ------------------------------------------------ |
| `agent_team.*`                      | `scope.runId/projectId/terminalSessionId`                                                            | 是    | 所属 `AgentTeamRun` 同名字段                                                    | 已有；worker/case 也必须携带                     |
| `agent_team.worker.* / case.*`      | `scope.operationId/panelId/tmuxPaneId`                                                               | 是/否 | 所属 dispatch 的稳定 ID 与实际 Worker panel/pane                                | operation/panel 必填；非 tmux 时 pane 为空       |
...
| `agent_team.run.*`                  | `externalRefs[role=run]`                                                                             | 否    | 状态 durable write 后的 run snapshot locator + canonical digest                 | 需注册 resolver；Ref 不替代状态 Fact             |
| `agent_team.worker.dispatched`      | `payload.workerId/role`                                                                              | 是    | `AgentTeamWorker.id/role`                                                       | 已有                                             |
...
| `agent_team.worker.result_recorded` | `payload.workerId/role`                                                                              | 是    | active worker 与已验证 outbox identity                                          | 已有原值                                         |
...
| `agent_team.case.dispatched`        | `payload.caseId`                                                                                     | 是    | 本次成功投递 prompt 实际包含的 Case                                             | 已有 Case ID；需 prompt builder 返回列表         |
...
| `source.events_dropped`             | `payload.affectedProducerName/Version/RuntimeChannel/RuntimeSurface/InstanceId/BootId/BootStartedAt` | 是    | compact loss ledger 从原 bundle 复制的完整 producer epoch manifest              | reporter 重启/升级后也不能用当前 envelope 覆盖   |
...
| 同上                                | `payload.reasonCode/priorityClass`                                                                   | 是    | 实际执行 sampling/quota/retention drop 的分支常量                               | critical 仅允许因 7 天 spool retention 到期丢弃  |
...
- Hub 是唯一写入者，按 batch 使用 `BEGIN IMMEDIATE`：验证/脱敏后，在同一事务内完成 eventId/sequence + fingerprint 幂等校验、Fact、Content ciphertext、Ref/link、producer cursor 与 gap 更新；`COMMIT` 成功后才 ACK。
- Facts 不提供 update API；只有 retention/delete capability 可以在审计事务中删除行。字段修正通过新事件或 schema upcaster 查询，不覆盖历史 row。
- Query/回复/命令/tool excerpt 以 AES-256-GCM ciphertext BLOB 存 `activity_contents`；nonce/auth tag 与 key identity/version 同行保存，key bytes 只在 macOS Keychain。每行 AAD 绑定 `(table, primaryId, ownerNamespace, kind/authority, keyVersion)`，nonce 使用每个 key 下唯一的 96-bit CSPRNG 值，防止 ciphertext/nonce/tag 整组跨行置换。轮换时由 Hub 有界重加密，WAL 中只出现 ciphertext，不写明文正文。
- 大型 Thread、scrollback、run/outbox、report、截图和 trace 不写 BLOB，只在 `external_refs` 保存加密 locator 与版本/哈希。
- 定期执行 passive checkpoint；达到 WAL 阈值时做受控 truncate checkpoint。首期不创建自动备份、副本或 shadow Fact store，避免绕过 7/30 天留存与用户删除。
- Fact 与 Content 的 retention anchor 固定为 `min(occurred_at_ms, ingested_at_ms)`；延迟 replay 不能重置 TTL。Hub 收到已过 30 天的事实时不写 `behavior_facts`，而是在同一事务写单 sequence `source_gaps` terminal receipt（eventId + normalized fingerprint + `retention_expired_before_ingest`）、推进 `highest_contiguous_accounted_sequence` 并返回 terminal ACK。ACK 丢失时相同 fingerprint 返回同一结果，不同 fingerprint 报冲突；7 天内容已过期但 Fact 未过期时只写 digest/link 与 `expired` tombstone，不写 ciphertext。
- Retention 始终用各表 `expires_at_ms <= now` 的索引做 keyset batch，不能用 sweep 水位跳过迟到 row。Fact 事务先删 link 再删 Fact，随后 GC 无引用的 Content/Ref；source gap/terminal receipt 按自身 30 天 expiry 独立删除，最后清理已无 Fact/gap 的 producer row 并 incremental vacuum；`retention_sweeps` 只记录执行 telemetry。
- Producer spool 同样使用小型 SQLite 与字段加密，但单条 bundle 可能含 7 天 Content，因此整个未 ACK bundle 最多保留 7 天；到期删除前先写不含正文的 30 天 loss range。Hub 恢复后用 `source.events_dropped(reason=retention_expired)` 明确暴露缺口，不假装离线超过 7 天仍能补齐 critical Fact。
...
### Producer spool SQLite

每个 `<producer-instance>.sqlite` 只有两个 transient 表。它们保证业务路径不等待 Hub，但不是第二份 canonical Fact store：成功 ACK 后删除，7/30 天到期也删除；查询页面从不直接读取 spool。
...
CREATE TABLE spool_events (
  ...
  producer_sequence INTEGER NOT NULL,
  event_id TEXT NOT NULL UNIQUE,
  sdk_bundle_sha256 TEXT NOT NULL,
  occurred_at_ms INTEGER NOT NULL,
  retention_anchor_ms INTEGER NOT NULL,
  expires_at_ms INTEGER NOT NULL,
  priority TEXT NOT NULL,
  ...
  PRIMARY KEY (producer_instance_id, producer_boot_id, producer_sequence),
  CHECK (producer_sequence >= 1),
  CHECK (length(sdk_bundle_sha256) = 64),
  CHECK (producer_boot_started_at_ms >= 0 AND producer_boot_started_at_ms <= occurred_at_ms),
  CHECK (occurred_at_ms >= 0 AND enqueued_at_ms >= 0),
  CHECK (retention_anchor_ms <= occurred_at_ms AND retention_anchor_ms <= enqueued_at_ms),
  CHECK (expires_at_ms >= retention_anchor_ms),
  CHECK (encryption_key_version >= 1),
  CHECK (attempt_count >= 0),
  CHECK (last_attempt_at_ms IS NULL OR last_attempt_at_ms >= enqueued_at_ms)
) STRICT;

CREATE INDEX spool_events_expiry_idx
  ON spool_events (expires_at_ms, producer_sequence);

CREATE TABLE spool_loss_ranges (
  loss_id TEXT PRIMARY KEY,
  affected_producer_name TEXT NOT NULL,
  affected_producer_version TEXT NOT NULL,
  affected_runtime_channel TEXT NOT NULL,
  affected_runtime_surface TEXT NOT NULL,
  affected_instance_id TEXT NOT NULL,
  affected_boot_id TEXT NOT NULL,
  affected_boot_started_at_ms INTEGER NOT NULL,
  first_sequence INTEGER NOT NULL,
  last_sequence INTEGER NOT NULL,
  dropped_count INTEGER NOT NULL,
  first_dropped_at_ms INTEGER NOT NULL,
  last_dropped_at_ms INTEGER NOT NULL,
  reason_code TEXT NOT NULL,
  ...
  expires_at_ms INTEGER NOT NULL,
  CHECK (first_sequence >= 1 AND last_sequence >= first_sequence),
  CHECK (affected_boot_started_at_ms >= 0),
  CHECK (dropped_count = last_sequence - first_sequence + 1),
  CHECK (first_dropped_at_ms >= 0 AND last_dropped_at_ms >= first_dropped_at_ms),
  CHECK (expires_at_ms >= last_dropped_at_ms),
  CHECK (
    (report_event_id IS NULL AND report_boot_id IS NULL AND report_sequence IS NULL AND reported_at_ms IS NULL)
    OR
    (
      report_event_id IS NOT NULL
      AND report_boot_id IS NOT NULL
      AND report_sequence >= 1
      AND reported_at_ms >= last_dropped_at_ms
    )
  )
) STRICT;

CREATE INDEX spool_loss_expiry_idx
  ON spool_loss_ranges (expires_at_ms, loss_id);
```

| 表                  | 字段                                                                                                                                                 | 来源类别     | 原始来源与生成规则                                                                                            |
| ------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- | ------------ | ------------------------------------------------------------------------------------------------------------- |
| `spool_events`      | `producer_name/version/runtime_channel/runtime_surface/producer_instance_id/producer_boot_id/producer_boot_started_at_ms/producer_sequence/event_id` | SDK          | 与待发送 typed event 完全相同的 durable manifest/identity；retry 不变                                         |
| 同上                | `sdk_bundle_sha256`                                                                                                                                  | SDK          | 对 Event + Content/Ref parts 的 canonical bytes 计算，用于本地损坏/冲突检查                                   |
| 同上                | `occurred_at_ms`                                                                                                                                     | Producer     | 业务边界原始时间，不由 spool 重写                                                                             |
| 同上                | `enqueued_at_ms/retention_anchor_ms/expires_at_ms`                                                                                                   | SDK/Registry | enqueue 时取时；anchor 为 `min(occurred,enqueued)`；bundle TTL 最多 7 天                                      |
| 同上                | `priority`                                                                                                                                           | Registry     | 对应 event schema 固定值                                                                                      |
| 同上                | `bundle_ciphertext/nonce/auth_tag/encryption_key_id/encryption_key_version`                                                                          | SDK          | Producer storage adapter 用 Keychain key 加密；AAD 绑定 producer epoch/sequence/event/fingerprint/key version |
| 同上                | `attempt_count/last_attempt_at_ms`                                                                                                                   | SDK          | 每次真实向 Hub 发起 ingest 后事务更新；不代表 Hub 已收到                                                      |
| `spool_loss_ranges` | `loss_id`                                                                                                                                            | SDK          | 首次 durable 记录连续 loss range 时生成 UUID，retry/report 复用                                               |
| 同上                | `affected_producer_name/version/runtime_channel/runtime_surface`                                                                                     | SDK          | 从被删除 `spool_events` 的原始 envelope manifest 复制；Reporter 重启/升级后也不改写                           |
| 同上                | `affected_instance_id/affected_boot_id/affected_boot_started_at_ms/first_sequence/last_sequence/dropped_count`                                       | SDK          | 被实际 sampling、quota 或 retention 删除的 epoch/连续 sequence 区间                                           |
| 同上                | `first_dropped_at_ms/last_dropped_at_ms/reason_code/priority_class`                                                                                  | SDK/Registry | 实际 drop 分支时间与闭集 reason/priority；禁止从 Hub gap 反推                                                 |
| 同上                | `report_event_id/report_boot_id/report_sequence/reported_at_ms`                                                                                      | SDK          | 物化 `source.events_dropped` 时一次生成并 durable，ACK retry 不变；尚未物化时全为 `NULL`                      |
| 同上                | `expires_at_ms`                                                                                                                                      | Registry/SDK | loss metadata 最多保留 30 天，不含原始正文                                                                    |

删除 `spool_events` 前必须在同一 Producer SQLite transaction 先插入/合并 `spool_loss_ranges`；loss table 与其 report event 使用独立预留 quota，不受普通 bundle 配额挤占。若 loss ledger 自身无法 durable，系统只能让 Hub 后续显示 unknown gap，不能伪造 reason。

### 默认留存

| 数据                                                |                 默认 | 清理语义                                                                          |
| --------------------------------------------------- | -------------------: | --------------------------------------------------------------------------------- |
| Query/回复/命令正文、excerpt ciphertext、quarantine |                 7 天 | 清空 SQLite ciphertext；link 保留 digest/size，Content tombstone 保留 kind/status |
| 未 ACK 的 Producer spool encrypted bundle           |                 7 天 | 到期前写 compact loss range，再删除 bundle ciphertext                             |
| 规范化 Fact、关系、Refs、Sources gap 与索引         |                30 天 | 按 `expires_at_ms` 有界删除 SQLite rows，更新 retention sweep state               |
| Producer spool loss ranges                          |                30 天 | ACK `source.events_dropped` 后可提前删；绝不含 Query/回复/locator 原文            |
...
- 交付语义：at-least-once + eventId/producer sequence 去重；同 ID 只有 fingerprint 一致才幂等 ACK，不承诺 exactly-once 网络传输。
...
- WAL/进程崩溃：启动时由 SQLite recovery 恢复已 commit 事务；`integrity_check/foreign_key_check` 失败则 fail closed 并明确报告数据不可用，不能跳过损坏 row、从 JSONL 猜回或静默创建“看似完整”的事实库。
...
- Backpressure：429 + retry-after；critical 在其 retention 期内不因 quota/sampling 丢弃，normal spool，sampled 可采样并报告 loss。
...
- SQLite 中的 Content 与 locator 使用 AES-256-GCM field encryption；key 存 macOS Keychain；根目录 `0700`，database/WAL/SHM 文件 `0600`。
...
WS   /v1/events/stream?afterHubOffset=...
```

Timeline 展示分页在第一页冻结 `asOfHubOffset`，随后使用 opaque keyset `(occurredAt, hubOffset)` 且始终限制 `hubOffset <= asOfHubOffset`，不能用 offset pagination。Change feed、WS 和 Learning 增量 watermark 只按单调 `hubOffset` 前进；若把 `occurredAt` 当增量 cursor，迟到事件会永久漏读。所有 query 返回 `coverage`：source watermarks、gap ranges、SQLite commit/checkpoint 状态、expired/missing ref counts。
...
- 新增 `activity-hub/` workspace package：discovery/auth、batch ingest、SQLite canonical storage、encrypted Content BLOB、refs、retention、quarantine、query/WS。
...
- 新增 producer SDK 与上文两表结构的 per-producer SQLite spool。
- 在 Web/Desktop 增加 Sources/Data Policy 最小只读面板。

验收：Hub 下线/replay、重复、gap、schema reject、transaction rollback、WAL crash recovery、index rebuild、7/30 天 purge、Stable/Beta 同库隔离字段、5 GB quota 策略。

### P2：核心 Producer 与真实 Facts/Timeline

改动范围：

- Backend：在 producer bootstrap、terminal application service 与 zsh command marker 边界主动记录。
...
- 不提供历史 backfill；旧 App Server event、当前 run snapshot 或 Thread metadata 不能伪装成“当时主动记录”的实时行为事实。
