...
6. 拼成文本；默认交给 `rw feishu notify` 通过应用机器人发送，显式 webhook transport 时调用旧群机器人 Webhook。
...
- **`notifyFeishu(payload, source, terminalSessionId, terminalPanelId)`**：调 `~/.runweave/hooks/feishu_stop_notify.sh`，存在才执行，stdin 传带 Terminal 路由上下文的 payload。**无 codex 特判**。
