...
> 所有来源（codex / coco / trae / claude）的桌面通知 + 声音 + 飞书，全部由 Runweave 的 launcher `~/.runweave/bin/runweave-hook-bridge` 负责，不再有任何 `source === "codex"` 特判。

```text
AI CLI 任务完成
  -> CLI hook 执行 runweave-hook-bridge --source <src>
...
飞书发送脚本是 toolkit hook 资产的一部分，不再依赖本机 `~/.codex/hooks/`：

- **源文件**：`plugins/toolkit/hooks/feishu_stop_notify.sh`。
- **运行副本**：`electron/resources/hooks/feishu_stop_notify.sh`，由 `pnpm toolkit:sync` 从 toolkit 同步，随 `resources/**` 打进 asar（与 app 图标同机制，无需改打包配置）。
- **安装拷贝**：Electron 启动时 `installNotifyAssets()` 把它 `copyFile` 到 `~/.runweave/hooks/feishu_stop_notify.sh` 并 `chmod 0755`。
- **launcher 调用**：`notifyFeishu` 调 `~/.runweave/hooks/feishu_stop_notify.sh`，存在才执行，stdin 传入原始 payload（注入正确 `source`）。
...
webhook 地址和密钥**不进仓库**，放在本机 `~/.runweave/feishu_notify.env`，由脚本 `source` 进来：
...
飞书脚本依赖 `jq` 解析 payload，依赖 `openssl`（默认 `/opt/homebrew/bin/openssl`，否则回退 `PATH`）做加签。**macOS 默认不带 `jq`**，缺失时脚本会在 `~/.runweave/feishu_notify.log` 记录 `skip: jq missing` 并静默跳过——表现为 env 配好了却收不到飞书消息。需 `brew install jq`。排查飞书不发时，先看该日志。

## launcher 关键函数

launcher 源码放在 `plugins/toolkit/hooks/runweave-hook-bridge.cjs`，运行副本在
`electron/resources/hooks/runweave-hook-bridge.cjs`。Electron 启动时
`writeLauncherScript()` 会把运行副本安装到 `~/.runweave/bin/runweave-hook-bridge`；
仅当资源文件缺失时才回退到 `electron/src/hooks/hook-launcher-script.ts` 的
`buildLauncherScript()`。
...
- **`notifyFeishu(payload, source)`**：调 `~/.runweave/hooks/feishu_stop_notify.sh`，存在才执行，stdin 传 payload。**无 codex 特判**。
- **source 归因**：插件级 hook 优先使用显式 `RUNWEAVE_HOOK_SOURCE`；没有显式值时，按正在执行的插件目录判定（例如 `.codex/...` -> Codex），最后才看外层 `CODEX_PLUGIN_ROOT` / `CLAUDE_PLUGIN_ROOT` 等环境变量。这样即使 Codex 终端继承了 Claude 相关环境变量，也不会把飞书文案误写成 “Claude 任务完成”。

## codex 旧条目清理

早期接入曾在 `~/.codex/hooks.json` 注册过 `notify.sh` / `feishu_stop_notify.sh` 以及 Runweave launcher hook。Codex 迁到 Toolkit 插件级 `hooks.json` 后，`installCodexHooks()` 只作为兼容清理层：移除命令指向 `notify.sh` / `feishu_stop_notify.sh` 的旧通知条目，并清理带 `_runweaveManaged` 或旧 bridge 路径的 Runweave 全局条目（保留第三方 hook），避免双发。
...
- launcher / 飞书脚本源码：`plugins/toolkit/hooks/`
- Codex / Trae CLI 插件级 hook 注册：`plugins/toolkit/hooks.json`
- launcher 安装 + 脚本拷贝：`electron/src/hooks/hook-installer.ts`（`writeLauncherScript` / `installNotifyAssets` / `installCodexHooks` / `installTraeHooks`）
- Codex/Trae 旧全局配置兼容清理：`electron/src/hooks/hook-installer-config.ts`
