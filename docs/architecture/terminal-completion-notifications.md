...
仅当资源文件缺失时才回退到 `electron/src/hooks/hook-installer.ts` 的
`buildLauncherScript()`。
...
- launcher 安装 + 脚本拷贝 + Codex/Trae 旧全局配置兼容清理：`electron/src/hooks/hook-installer.ts`（`writeLauncherScript` / `installNotifyAssets` / `pruneSupersededCodexHooks` / `installCodexHooks` / `installTraeHooks`）
