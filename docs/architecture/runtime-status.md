...
右上角入口按关注级别显示：关键能力故障显示红色“异常”，飞书这类可选能力故障、
配置缺失、状态未上报或版本不支持显示黄色 `Warning`。异常和 Warning 分别计数，
同时出现时红色优先；能力域内部分项目正常也不能掩盖其他项目的配置缺失。
显式停用不计为 Warning。首次加载已有关键异常也会提示，同一异常持续期间不重复弹出；
Warning 仅在入口和面板展示，不发送系统错误通知。原因和影响在状态面板中查看。

Backend 未收到飞书 Bridge 报告时只表达“尚未收到状态、无法判断”，不推断为配置缺失。
关注级别独立于 owner 上报的生命周期状态，不修改协议状态枚举。
