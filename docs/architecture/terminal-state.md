...
`TerminalState` 不消费 completion event/feed，也不把完成提醒作为状态来源。`/internal/terminal-completion` 只负责“非当前终端里的 AI 任务完成提醒”，例如终端 tab 小绿点。状态服务在状态变化后会向全局 `TerminalEventService` 记录 `terminal_state_changed` envelope，由 `/ws/terminal-events` 推送给 Web/App；这个推送是状态结果的分发通道，不是状态判定来源。
...
- App 通过全局 `/ws/terminal-events` 接收 `terminal_state_changed` 事件刷新列表和终端详情状态；`/api/terminal/session/:id/state` 仍作为打开详情、调试和断线兜底查询入口。仅当 `terminalState.state === "agent_running"` 时展示 Stop。
