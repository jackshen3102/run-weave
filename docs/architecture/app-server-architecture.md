...
App Server 是 Runweave 在本机上的全局事件中心。它独立于 Desktop backend、
Electron shell、CLI 和 hook 进程运行，负责把来自不同进程的本地事件持久化并实时分发。

当前 app-server 只做事件中心和本机服务发现，不管理 terminal，不启动 backend，不拥有
浏览器 profile，也不替代 backend 现有的 terminal websocket。
...
| 组件                      | 代码入口                                                  | 职责                                                                          | 非职责                          |
| ------------------------- | --------------------------------------------------------- | ----------------------------------------------------------------------------- | ------------------------------- |
| app-server process        | `app-server/src/index.ts`                                 | 启动 HTTP/WS server、加载 token、初始化事件存储、写 singleton lock            | 不启动 backend，不管理 terminal |
...
| event schema              | `packages/shared/src/app-server-events.ts`                | 定义事件 envelope、source、scope、stream message 类型                         | 不决定 handler 语义             |
| HTTP API                  | `app-server/src/http-server.ts`                           | 鉴权、Origin 校验、事件写入和查询                                             | 不保存 consumer cursor          |
...
4. `app-server/src/index.ts`、`event-store.ts`、`http-server.ts`、`websocket-server.ts`：
   理解服务端事件中心。
5. `packages/runweave-cli/src/commands/app-server.ts`、`electron/src/app-server-cli.ts`：
   理解启动、重启和 packaged runtime 接入。
6. `backend/src/app-server/*` 与 hook bridge：理解 producer/consumer 和 fallback。
