...
网关仅提供统一通知接口。电量 Backend 显式订阅 `battery.low`，通过 `POST /v1/notifications`
提交标题、正文和稳定 eventId；网关不负责电量阈值、模板或轮次。
...
Backend 持久化待发记录，首次发送前固定通知内容与发生时间，再核对当前绑定、档位和时效后提交。
重试复用同一事件内容，样本变化仅用于取消判断。接电、过期、授权失效或进入更低档位会取消待发旧档。
