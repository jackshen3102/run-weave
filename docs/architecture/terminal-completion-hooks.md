...
旧 tmux pane 如果创建于这套环境注入之前，就没有这些变量。此时 launcher 因缺少 `RUNWEAVE_*` 身份而静默退出，既不点亮 tab，也不发通知。
...
4. 读取 `RUNWEAVE_HOOK_ENDPOINT`、`RUNWEAVE_HOOK_TOKEN`、`RUNWEAVE_TERMINAL_SESSION_ID`。缺任一则静默退出，不通知也不上报。
5. 仅在身份校验通过后，发出桌面/飞书通知（见下）并向 backend POST completion event。
6. 所有错误静默失败，不影响原 CLI。

第 4 步的身份门禁是关键边界。hook 安装进用户全局 Claude/Codex/Trae 配置，会覆盖所有 AI CLI（含 Runweave 之外的终端）。因此 launcher 的所有副作用——通知和上报——都只在 pane 带有 `RUNWEAVE_*` 身份（即从 Runweave terminal 启动）时发生；外部终端的 AI CLI stop 事件静默退出。

launcher 在上报之外还承担一层“完成通知”职责：身份校验通过后调用 `notifyDesktop()`（macOS `osascript` + `afplay`）和 `notifyFeishu()`（转调 `~/.browser-viewer/hooks/feishu_stop_notify.sh`）。通知的 channel 细节、飞书脚本与配置见 [terminal-completion-notifications.md](./terminal-completion-notifications.md)。
