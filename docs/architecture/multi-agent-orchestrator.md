...
Multi-Agent Orchestrator 是 Runweave 在单个项目内驱动多个终端 agent 协作的控制面。它把“主 agent 决策、worker agent 执行、结果自动回流”收敛到后端状态、终端输入队列和本地 JSON 文件，不引入外部 LLM API。

## 当前边界

- 主 agent 是一个真实的 Runweave terminal session。启动 run 时，后端会创建或复用该终端，确保对应 CLI agent 就绪，然后把启动提示词投递进去。
- worker 也是 terminal session。角色只描述职责、启动命令和提示词；运行时再绑定到新建或复用的终端。
- 后端是控制面和结果路由器，不替代主 agent 做智能决策。它负责保存 run package、创建或复用终端、投递 prompt、接收 completion event、解析 worker 结果，并把结果直发回主 agent。
- 结果回流不等待主 agent 空闲。worker completion 到达后，`OrchestratorService` 会把结构化结果作为新输入投递到主 agent 终端；顺序由终端输入队列保证。
...
创建和派发接口支持 `binding.mode = "new" | "reuse"`。复用终端时提供已有 `sessionId`；新建终端时按角色或主 agent 的 terminal 配置创建。

## 事件回流

1. worker 完成后触发既有 terminal completion event。
2. Orchestrator 结果路由器根据 completion 事件读取 outbox；如果 outbox 缺少 run 信息，会结合 dispatch sidecar 和终端 scrollback 做兜底解析。
3. 如果解析到 `runId` 且 completion 不是主 agent 自己发出的，后端更新对应 goal 和 timeline。
...
Web terminal 的 sidecar panel 包含 Orchestrator tab，用于配置角色、预览启动 prompt、创建 run、查看 timeline 和注入人工提示。人工注入是显式输入，不会绕过主 agent 决策；后端只负责把提示投递到主 agent 终端。

`paused` run 会记录 worker result，但不会自动直发回主 agent。用户需要恢复状态或注入提示后继续。
...
- outbox 是首选结果来源；缺失或格式不完整时会兜底读取 sidecar 与 scrollback，但摘要质量取决于终端尾部内容。
