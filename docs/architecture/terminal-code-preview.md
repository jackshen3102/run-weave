...
选择 `Files` 后，右侧显示 Spotlight / Cmd+P 风格的文件搜索面板。用户可以按文件名或相对路径模糊搜索当前项目路径内文件，选中后进入文件预览。project 内已存在的普通文本/代码文件可以编辑；绝对路径只能手动完整输入并打开，不参与模糊搜索，返回 `base: "filesystem"` 时始终只读。
...
- `Preview` 工具内部再用二级 Tab 切换 `Review changes` 和 `Files`。
