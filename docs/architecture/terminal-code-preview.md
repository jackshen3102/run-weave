...
- `frontend/src/components/terminal/terminal-submit-popover.tsx`：Terminal 顶栏里的 Git Submit 辅助入口，负责把结构化提交任务 prompt 发送到当前 active terminal 中的 AI agent；它不直接执行 Git，也不新增后端 Git executor。
...
- `packages/shared/src/terminal-protocol.ts`：Terminal 共享协议已包含 Preview search/file/git changes/file diff 和 directory tree 类型。
...
- Preview 面板不做 git commit、stage、unstage 等写操作；提交辅助只通过 Git Submit 把 prompt 发给当前 terminal 中已运行的 AI agent。
...
默认状态下，Terminal 保持全宽。桌面端在 Terminal session bar 右侧提供 `Preview` 按钮，位置靠近 `New Terminal`。当前实现中，点击 `Preview` 会直接打开右侧 Sidecar，并恢复当前 project 上次使用的 Preview 工具状态；首次打开默认落在 `Review changes`。

Preview 工具内部再区分两个稳定任务：

- `Review changes`：优先查看 staged / working changes，左侧按分组列出文件，右侧默认显示 diff。
...
`Preview` 按钮是 Sidecar 打开入口，不再承担预览任务切换。
...
Terminal 顶栏右侧提供 `Git Submit` 图标入口，位置在 project tabs 之后、Preview 入口之前。它是提交任务的 prompt 投递器，不是 Git 执行器。

当前实现边界：

- 只在桌面端展示；移动端保持监控和输入为主，避免小屏误触。
- 弹层提供 `Quick`、`Clean PR`、`Force Lease`、`Yolo` 四种任务语义，生成中文提交任务 prompt。
- `Send to AI` 通过现有 terminal input API 向 active terminal 发送 prompt 并回车；发送前会把多行 prompt 压成单条输入，避免终端逐行提前执行。
- `Copy` 仍复制原始多行 prompt，便于用户人工检查或粘贴。
- Prompt 明确要求 agent 先检查 `git status`、`git diff`、staged diff 和最近提交，再自行总结 commit title/message。
- 默认要求只 stage 属于本次任务的文件；只有用户选择 `Include all changes` 或 `Yolo` 时，才授权 `git add -A` 覆盖 modified、deleted、renamed 和 untracked。
- 前端不读取 Git 状态、不提交、不 push、不处理 rebase 冲突；这些仍由当前 terminal 中运行的 Codex、Coco 或其他 AI agent 完成。
- `Force Lease` 模式只允许在必要且说明原因时使用 `--force-with-lease`；prompt 明确禁止裸 `git push --force`。
- 入口依赖 active terminal session。没有 active terminal 时，只展示不可用提示，不创建后台任务。

该入口与 Preview 的关系是同一 Terminal 工作台中的轻量协作工具：Preview 帮用户看文件和 diff，Git Submit 帮用户把提交意图投递给当前 agent。两者都不把前端扩展成完整 IDE 或后端 Git 调度系统。
