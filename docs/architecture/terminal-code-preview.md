...
- `Files`：搜索当前 project path 下的文件，或显式输入完整路径后只读预览。

选择 `Files` 后，右侧显示 Spotlight / Cmd+P 风格的文件搜索面板。用户可以按文件名或相对路径模糊搜索当前项目路径内文件，选中后在右侧只读预览；绝对路径只能手动完整输入并打开，不参与模糊搜索。
...
- Markdown 文件的 Body 当前默认使用 `Preview` 视图；用户切到 `Split` 后，左侧显示只读原始 Markdown，右侧显示 rendered Markdown 预览。路径栏右侧显示 `Source` / `Split` / `Preview` 切换。
...
- Markdown 当前默认以 `Preview` 打开；切到 `Split` 后左侧是 readonly Monaco source，右侧是 rendered preview。
