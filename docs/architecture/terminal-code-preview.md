...
说明 Runweave Terminal 中轻量代码预览、Terminal Browser 和多项目原型轮巡库的产品边界、交互形态与后端协议。
...
- 文件树只作为 Preview 内的按需 Explorer，用于快速浏览当前 project path；不升级为完整 IDE 工作区
...
- `frontend/src/components/terminal/terminal-preview-panel-shell.tsx`：右侧 Sidecar 外壳层，负责 Header、`Preview` / `Prototypes` / `Browser` / `Agent Team` 工具 tab、路径栏和视图切换控件。
- `frontend/src/components/terminal/terminal-prototype-gallery.tsx`：多项目原型轮巡 UI，按 Runweave project 分组展示 `docs/prototypes`，并在受限 iframe 中加载当前原型。
...
- 注释 runtime 注入到目标 BrowserView 页面内，负责 hover 高亮、点击锁定元素、评论输入、编号 marker、编辑和删除；React 外层只承载工具栏状态和提交动作。
- 新建评论输入框按用户点击坐标定位，而不是按目标元素矩形定位；靠近视口边缘时会水平 clamp，并在下方空间不足时上翻，避免输入框溢出 BrowserView。
- 提交时 Electron 捕获带 marker 的 BrowserView 截图，前端通过 terminal clipboard image route 保存截图，再把图片文件路径写入 prompt。
...
2. 默认关闭，不影响终端主体验。
3. `Explorer` 只展示当前 project path 下的懒加载目录树，作为 Preview 的轻量入口，不提供全局索引、拖拽、目录操作或 LSP 能力。
4. 在 `Files` / `Explorer` 中对 project path 内已存在的普通文本/代码文件提供显式编辑保存；Changes、Diff、图片、二进制、大文件和 project 外绝对路径保持只读。
5. 对 project 内普通文件提供受限删除和重命名；对 `Review changes` 中的单个文件提供显式 `Reset Changes`，staged 变更只 unstage，working 变更会丢弃工作区改动，untracked 普通文件会删除。不支持目录、批量操作、project 外绝对路径或 git stage/commit。
...
- 不做完整 IDE 文件树、全局搜索、符号索引、LSP diagnostics；当前 Explorer 只做受限的一层懒加载目录浏览。
- 不做新建、目录移动、批量重命名、格式化、git stage 或 commit；文件写操作只覆盖 project 内受支持的普通文件，以及 `Review changes` 中经确认的单文件 reset。
- 不对 project 外绝对路径、目录、二进制、大文件或只读文件做保存、删除、重命名。
- Preview 面板不做 git commit、stage、checkout 或批量 reset。单文件 `Reset Changes` 只在 `Review changes` 文件项上出现，staged 文件执行 unstage，working 文件丢弃工作区改动，untracked 普通文件删除；提交类提示复用快捷指令，不保留单独提交入口。
...
默认状态下，Terminal 保持全宽。桌面端在 Terminal 顶栏右侧的 `...` 更多菜单中提供 `Preview` 与 `Prototypes` 入口。点击后直接打开同一个右侧 Sidecar 并切到对应工具；`Preview` 首次打开默认落在 `Review changes`。

Preview 工具内部再区分两个稳定任务：

- `Review changes`：优先查看 staged / working changes，左侧按分组列出文件，右侧默认显示 diff；每个文件项提供确认式 `Reset Changes`，用于处理单个 staged、working 或 untracked 文件。
- `Explorer`：按需浏览当前 project path 下的一层目录树，展开目录时才请求后端，点击文件后在右侧预览或编辑。
- `Files`：搜索当前 project path 下的文件，或显式输入完整路径后预览。project 内文本/代码文件可编辑并显式保存，project 外绝对路径只能只读打开。
...
项目来源只有一份：`TerminalSessionManager.listProjects()`。对每个已登记项目，后端只扫描 `<project.path>/docs/prototypes` 的一级真实目录，并返回以下状态：
...
验收入口见 `docs/testing/browser/prototype-gallery-preview-test-cases.md`。
...
打开 Markdown 文件时，右侧面板进入 Markdown view。当前实现默认显示 `Preview`，用户可切到 `Source` 或 `Split`；顶部路径栏右侧提供 `Source` / `Split` / `Preview` 切换。
...
切到 `Source` 后，只显示 Monaco Editor 中的 Markdown 源文。project 内文件可编辑；只读文件仍以 readonly Monaco 展示。用户可以随时切回 `Split` 或 `Preview`。
...
打开独立 `.svg` 文件时，右侧默认展示 SVG 预览，而不是源码。路径栏右侧提供 `Preview` / `Source` 切换；`Source` 用 Monaco 显示 SVG XML，project 内 SVG 可编辑保存，project 外 SVG 只读。

![SVG file preview](assets/terminal-code-preview-svg-preview.svg)

![SVG file source](assets/terminal-code-preview-svg-source.svg)

打开独立图片文件时，右侧默认展示图片预览。图片预览支持打开全屏 lightbox，并在 lightbox 内缩放、平移和关闭；Web 预览与 App 图片审阅复用 `@runweave/common/terminal` 的 `RunweaveImagePreview` / `RunweaveImageLightbox`，但仍通过显式子路径导入，不增加 `@runweave/common` 根导出。

![Image file preview](assets/terminal-code-preview-image-preview.svg)

查看提交前变更时，右侧显示 Changes 面板和 Monaco Diff Editor。左侧只列本次变更文件，并按 `Staged Changes` / `Working Changes` 分组；Explorer 的目录树不会混入 Changes。
...
| 交互                                                   | 草图                                                |
| ------------------------------------------------------ | --------------------------------------------------- |
| 切到 `Files` 并搜索文件                                | `assets/terminal-code-preview-open-file.svg`        |
...
| 默认进入 `Review changes` 并查看 staged / working diff | `assets/terminal-code-preview-diff.svg`             |
...
`...` 更多菜单中的 `Preview` 项是 Sidecar 打开入口，不再承担预览任务切换。

当前交互规则：

- 第一次点击 `Preview`：直接打开右侧 Sidecar，并激活 `Preview` 工具。
- 如果当前 project 没有历史状态，默认落在 `Review changes`。
- `Preview` 工具内部用任务 Tab 切换 `Review changes`、`Explorer` 和 `Files`。
- `Review changes` 保留左侧 `Staged Changes` / `Working Changes` 文件列表，右侧显示当前 diff 或支持的 preview。
- `Explorer` 提供当前 project path 的受限懒加载目录树，点击文件后复用文件预览链路。
- `Files` 保留文件搜索和只读文件预览，不再依赖顶部菜单深跳。
- 关闭动作放在右侧面板 Header 的 `Close` 按钮中。

`Preview` 入口收纳在 desktop client mode 的 `...` 更多菜单中。mobile client mode 继续保持轻量 monitor 定位。
...
面板关闭时，不建议保留右侧边缘竖条作为主入口。主入口仍是 Terminal 顶栏 `...` 更多菜单中的 `Preview` 项，避免界面看起来像 IDE activity bar。

从 terminal 输出中点击文件路径打开预览可作为 v2 能力，不进入 v1。原因：

- xterm 里的选择、复制、链接点击容易冲突。
- 路径识别需要处理相对路径、绝对路径、行号、历史输出等边界。
- v1 的重点是轻量任务菜单和受限 Preview 面板，不演变成完整文件浏览器或 IDE。

如果后续支持，可考虑 `Cmd/Ctrl + click` terminal 中的文件路径，在右侧 Preview 打开对应文件。

Terminal 输出中的普通 `http:` / `https:` 链接仍由 xterm WebLinksAddon 负责打开。当前只额外注册一个窄范围 wrapped-link provider，用来补齐明确由 xterm 标记为 `isWrapped` 的跨行 URL；它不接管常规 URL 解析，也不负责文件路径预览。这个 provider 属于兼容 shim，后续如果确认 WebLinksAddon 已覆盖相同场景，应优先删除 shim，避免长期维护重复的 URL 正则、坐标映射和宽字符边界。
...
1. Header
   - 顶层工具：`Preview` / `Browser`
   - 状态 badge：`Read only`、加载中、错误
   - 操作：Refresh、Copy path、Close

2. Preview tool bar
   - 任务：`Review changes` / `Explorer` / `Files`
   - 当前 project 名称与 root path
   - 文件或 diff 选中后显示当前路径，以及 Markdown / SVG / Changes 对应的视图切换
...
7. 右侧显示文件预览。project 内普通文本/代码文件进入可编辑 Monaco，其他内容保持只读。

绝对路径可以允许，但必须由用户显式输入并按 `Enter` 打开。绝对路径不进入候选列表，不做模糊搜索，不进入最近列表。project 外绝对路径可作为只读文件或图片预览，不允许保存。
...
- 绝对路径解析到 project path 内时按 project 文件处理；解析到 project 外时返回 `base: "filesystem"`，只读预览并隐藏 Save。
...
- Header 显示 `Open file`、保存状态、`Updated ... ago`、`Refresh`、`Copy path`、`Close`。保存状态包括 `Read only`、`Editable`、`Unsaved`、`Saving...`、`Saved`、`Conflict`。
- Context bar 显示当前相对 project path 的路径，不常驻展示 project path 或 terminal `cwd`。
- 普通代码文件的 Body 使用 Monaco Editor；project 内可编辑文件按 `readonly: false` 开放编辑，其他文件保持只读。
- Markdown 文件的 Body 当前默认使用 `Preview` 视图；用户切到 `Split` 后，左侧显示原始 Markdown，右侧显示 rendered Markdown 预览。project 内 Markdown draft 会驱动右侧实时预览。路径栏右侧显示 `Source` / `Split` / `Preview` 切换。
...
编辑能力只属于 `Files` 模式下 project path 内已存在的普通文本/代码文件。后端读取文件时返回 `base`、`mtimeMs` 和 `readonly`：

- `base: "project"` 表示路径在当前 Terminal Project path 内；普通文本/代码文件可以编辑保存。
- `base: "filesystem"` 表示用户手动输入了 project 外绝对路径；可以预览文本或图片，但必须只读，不显示 Save。
- Changes、Diff editor、图片、二进制、大文件、目录和无 project path 的项目都保持只读。

保存是显式动作，不做自动写盘。用户编辑后进入 dirty 状态，点击 Header 的 Save 或按 `Cmd/Ctrl+S` 才调用保存接口。保存成功后更新当前 payload、`mtimeMs` 和 loaded content；保存失败时保留用户 draft。

冲突使用 mtime v1：打开文件时记录 `mtimeMs`，保存时把 `expectedMtimeMs` 传给后端。如果磁盘文件已变化且没有 `overwrite: true`，后端返回 `409`，UI 展示 `Conflict`，并提供 Reload 和 Overwrite。Reload 丢弃当前 draft 并重新读取文件，Overwrite 用当前 draft 覆盖磁盘。

切换文件、Refresh、关闭 Preview 或切换到 Changes 前，如果当前文件有 unsaved draft，需要先确认是否丢弃，不能静默丢失。
...
- 打开 Markdown 文件后，当前实现默认进入 `Preview` view，仍支持切到 `Source` 和 `Split`。
...
`Open file...` 的目标是轻量文件定位，不是文件浏览器。它采用更接近 VS Code `Cmd+P` 的紧凑交互：顶部一个输入框，下方直接显示结果列表，选中后打开文件预览。project 内文本/代码文件可编辑保存，其他内容只读。
...
- 绝对路径不参与搜索，只能完整输入后打开；project 外路径只读，不显示 Save。
- 搜索接口不返回绝对路径候选。
...
- `frontend/src/components/terminal/terminal-file-explorer.tsx`
- `frontend/src/components/terminal/terminal-file-tree.tsx`
- `frontend/src/components/terminal/terminal-file-tree-item.tsx`
...
type TerminalPreviewMode = "file" | "changes" | "explorer";
...
- `TerminalPreviewPanel` 直接通过 store selector 读取所需状态，并把交互动作下沉到 `useTerminalPreviewPanelActions`，把 UI 外壳和内容渲染分别下沉到 `TerminalPreviewPanelShell` 与 `TerminalPreviewPanelContent`。
- `TerminalPreviewMenu`、`TerminalOpenFileCommand` 继续只读取自身所需的 store 状态。
...
- `TerminalPreviewPanel` 由 `TerminalWorkspaceShell` 通过 `React.lazy` / dynamic import 加载。
...
- 显式绝对路径可以位于 project path 外；位于 project 外时返回 `base: "filesystem"` 且只读。
- 后端返回 normalized path、absolute path、解析基准、`mtimeMs`、`readonly` 和内容。
...
- 只允许保存当前 project path 内的已存在普通文件，不能写入 `base: "filesystem"` 的 project 外路径。
- 保存前复用 Preview 路径约束、目录 / 普通文件检查、binary 检查和大小上限。
- `expectedMtimeMs` 必填；mtime 不一致时返回 `409`，除非请求显式带 `overwrite: true`。
...
- project id 用于定位 project path。
- `path` 为空或省略时列出项目根目录；非空时必须解析在 project path 内。
- 只返回一层 children，不递归读取整个仓库。
- 目录优先、文件其次，同类按 basename 排序。
- 默认过滤重目录、构建产物、测试产物和敏感环境文件。
- `limit` 默认 500，最大 1000；超过时返回 `truncated: true`，前端应避免暗示目录完整。
- 文件 path 返回 400，不存在目录返回 404，project path 缺失返回 409，越界返回 403。
...
- 默认只读 git 命令；唯一例外是用户在 `Review changes` 文件项上确认 `Reset Changes` 后触发的单文件 reset。
- 设置超时，例如 5 秒。
...
- 不执行 stage、checkout、commit 或批量 reset。单文件 reset 必须先解析 project path 所在 git repo，并把请求路径限制在 project path 内；staged 变更只执行 `git restore --staged -- <path>`，working 变更执行 `git restore --worktree -- <path>`，untracked 只删除普通文件，目录和特殊文件拒绝处理。
...
后续如果需要移动端预览，可以单独设计为只读底部抽屉，并且不复用桌面 Explorer。
...
- 绝对路径手动输入打开，不参与搜索；project 外绝对路径返回 `base: "filesystem"` 并保持只读。
- `Changes`
...
- 独立 `.svg` 文件默认显示 SVG preview，可切换到 source；project 内 SVG source 可编辑保存。
...
- `Open file...` 可增加路径补全，但只补当前 project path 内候选；目录浏览继续收口在 Explorer。
...
- Preview 打开后的默认宽度是 60%，仍可拖拽调整，最大宽度保持 60%。
...
- `Open file...` 绝对路径只允许完整输入后打开，不参与 fuzzy search；project 外路径可预览但只读，不显示 Save。
...
- Markdown `Source` 和 `Split` 中的 source 区域复用 Monaco；project 内文件可编辑保存，project 外文件只读。
...
- SVG `Source` 模式复用 Monaco；project 内 SVG 可编辑保存，project 外 SVG 只读。
...
- Changes 左侧文件索引不扩展成项目文件树；目录浏览只在 Explorer 中出现。
...
- `preview/file` 遇到目录路径不返回正文；目录浏览只走受限的 `preview/directory` 接口。
...
6. Markdown 默认视图目前是 `Preview`。如果后续要改成 `Split`，需要同时更新 store 默认值、交互预期和验收描述。
