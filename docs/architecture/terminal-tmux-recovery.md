...
当前 WebSocket 层还会按连接数管理 tmux attach runtime：最后一个客户端断开时，后端只 dispose 本轮 node-pty attach runtime，并从 runtime registry 移除它；tmux server 中的 session 和用户任务继续存活。下一次 WebSocket 连接会重新 attach 同名 tmux session，并从 `tmux capture-pane` 获取新的初始快照，避免复用无人观看时留下的旧 attach runtime。
