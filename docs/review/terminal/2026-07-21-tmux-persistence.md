...
- 位置：`scripts/verify/agent-team/review-checkpoints/bootstrap/reconciliation.mjs:2`
...
- `scripts/update/operations.mjs` 和 `scripts/electron/local-update.mjs` 使用 `ps -axo pid=,comm=` 后只匹配 App bundle 内真实可执行文件。当前 Stable 实测识别 12 个 Runweave/Electron 进程，没有把 tmux 纳入结果；Stable 与 Beta 的 App 名也不会互相匹配。
