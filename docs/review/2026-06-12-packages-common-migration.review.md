...
- 当前决策：新增通用包名为 `@browser-viewer/common`，并从根导出 `.` 与 `./terminal`，但包内实际依赖 DOM 类型，语义上只服务 App/Web 前端公共层。
...
- 证据：
  ...
  - `packages/common/package.json:2` 使用宽泛包名 `@browser-viewer/common`。
...
- 更好的候选方案：
  ...
  2. 更强方案：将根导出收窄为空或仅保留显式子路径，例如 `@browser-viewer/common/terminal`、`@browser-viewer/common/styles/*`，避免 `@browser-viewer/common` 成为杂物入口。
  3. 平台/工具链方案：如果后续频繁漂移，可用 lint/import rule 限制 backend/electron/CLI 不能依赖 `@browser-viewer/common`，并要求新增 common 导出经过 allowlist。
...
- `fileToBase64`、`shellQuote` 已从 App/Web 原重复实现迁入 `@browser-viewer/common/terminal`，调用方包括 `app/src/pages/AppTerminalPage.tsx:6`、`frontend/src/components/terminal/use-terminal-emulator.ts:2` 到 `frontend/src/components/terminal/use-terminal-emulator.ts:7`。
