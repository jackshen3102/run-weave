...
- `app-server/src/state/reconciler.ts` 当前将 `readThreadStatus()` 返回的 `notLoaded` 在旧状态为 `running` 时转换为 `idle`。
- `app-server/src/codex/client.ts` 在 `thread/read` 未加载后会尝试 `thread/resume`，但恢复失败或仍不可用时依旧可能返回 `notLoaded`。
