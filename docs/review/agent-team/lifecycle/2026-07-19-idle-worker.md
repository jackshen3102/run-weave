...
- `docs/plans/agent-team/2026-07-19-idle-worker-lifecycle.md`
...
计划在 `docs/plans/agent-team/2026-07-19-idle-worker-lifecycle.md:30-40,44-51` 规定：current identity 为空时，只要 Pane、provider 与 lastThreadId 匹配，就接受 `Stop`。
...
计划 `docs/plans/agent-team/2026-07-19-idle-worker-lifecycle.md:106-108` 要求上线后重放原 Stop。重放时 Backend 只看新的到达顺序，无法证明该 Stop 晚于还是早于最近一次 UserPromptSubmit；若现场期间产生新 turn，旧 Stop 会覆盖新状态。
...
计划 `docs/plans/agent-team/2026-07-19-idle-worker-lifecycle.md:20` 已把 activeCommand 短暂清空导致 metadata 清理标为推测，后续实现步骤却没有继续验证 `panel-workspace.ts` 的 `shouldClearAgentThreadMetadata`、activeCommand source 与 recent activity/grace 时序。
