...
| 下发目标 → 为每个 worker 自动建 worktree | 只读 `git worktree list`；**文档明令不在 Runweave 内执行 `worktree add`**（原型基线的历史限制；当前边界见[Worktree 与 Race 生命周期](../../architecture/terminal-worktree-context.md)）                               | **需新写 + 越现有边界** | 保留但标注：真实实现要么新增受控 `git worktree add`、要么由 CLI/外部预建后被 registry 发现                                 |
