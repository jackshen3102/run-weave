...
6. 点击未确认 Completion 后，先成功打开目标 Session并返回 `opened` 或 `opened_with_panel_fallback`，再异步确认对应 `completionRevision`；Panel 失效时退回 Session并提示，仍可确认。Connection 或 Session 跳转失败时不确认；确认失败或挂起时保持提醒，不改变已经完成的跳转结果。
...
5. 在目标表面成功打开并完成本次 `openSlot` 后，如点击的是 Completion，异步调用现有 Session PATCH 写入 intent 携带的精确 `acknowledgedCompletionRevision`。

`openSlot` 的有界终态只表达目标表面是否打开。Completion 是否退役只以之后 Attention snapshot 中的 `acknowledgedCompletionRevision` 为准；确认失败或挂起时提醒继续存在，不能把已经返回的 `opened` 改写为失败或超时。若目标表面没有在 deadline 前打开，则返回失败或 `timed_out`，不确认 revision，迟到结果也不能再次完成请求。
...
- Completion 的退役事实写入现有 `acknowledgedCompletionRevision`；必须先成功打开目标 Session并结束 `openSlot`，再异步精确确认。只有后续 snapshot 已确认该 revision 才退役，确认失败或挂起时保持提醒。
...
- 点击 Completion 只有在 Session 成功打开并返回成功结果后才异步确认 revision；Panel 失效可以降级，Connection 或 Session 跳转失败不能确认，确认失败或挂起不能提前退役。
