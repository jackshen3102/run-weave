...
| 编号   | 场景                               | 层级                  | 必跑 |
| ------ | ---------------------------------- | --------------------- | ---- |
| WH-001 | Activity 新导航与默认视图          | UI                    | 是   |
| WH-002 | 同一 Terminal 关联两个 Thread      | API + UI              | 是   |
| WH-003 | 退出/重进 Codex 不拆 Terminal 档案 | 实际行为 + UI         | 是   |
| WH-004 | Terminal Journal 异构事件稳定排序  | API + UI              | 是   |
| WH-005 | Codex Thread Turn/消息按需读取     | App Server + API + UI | 是   |
| WH-006 | Codex Provider 不可用时降级        | API + UI              | 是   |
| WH-007 | Activity 数据源不可用时降级        | API + UI              | 是   |
| WH-008 | Facts 快照分页无重复/跳项          | API + UI              | 是   |
| WH-009 | Activity Fact/Content 详情语义     | UI                    | 是   |
| WH-010 | Inspector 宽屏内嵌、窄屏抽屉       | UI                    | 是   |
| WH-011 | Terminal 搜索、排序和稳定游标      | API + UI              | 是   |
| WH-012 | Multi-Agent Run 独立列表           | API + UI              | 是   |
| WH-013 | nextRoundIndex 与已呈现轮次区分    | API + UI              | 是   |
| WH-014 | 多轮、补充 Worker 与重试结构       | UI                    | 是   |
| WH-015 | 历史 Round 缺失与归属来源          | API + UI              | 是   |
| WH-016 | Acceptance Evidence 真实下钻       | API + UI              | 是   |
| WH-017 | 无事实字段不展示假数据             | UI                    | 是   |
| WH-018 | 非 Codex/未知 Thread Provider      | API + UI              | 是   |
| WH-019 | Work History 与 Thread detail 鉴权 | API                   | 是   |
| WH-020 | 禁止按时间建立跨对象关联           | 脚本 + API            | 是   |
| WH-021 | 原 Activity 子页面回归             | UI                    | 是   |
| WH-022 | 切换对象时旧请求不可覆盖新选择     | UI                    | 是   |
| WH-023 | Thread 上限与部分加载状态          | API + UI              | 是   |
| WH-024 | 对象删除/不存在后的恢复            | API + UI              | 是   |

## 5. 详细用例

### WH-001 Activity 新导航与默认视图
...
### WH-002 同一 Terminal 关联两个 Thread
...
### WH-003 退出/重进 Codex 不拆 Terminal 档案
...
### WH-004 Terminal Journal 异构事件稳定排序
...
### WH-005 Codex Thread Turn/消息按需读取
...
### WH-009 Activity Fact/Content 详情语义
...
### WH-013 nextRoundIndex 与已呈现轮次区分
...
### WH-016 Acceptance Evidence 真实下钻
...
### WH-018 非 Codex/未知 Thread Provider
...
### WH-019 Work History 与 Thread detail 鉴权
...
| 计划能力                       | 覆盖用例                               |
| ------------------------------ | -------------------------------------- |
| Terminal 以 session 为唯一档案 | WH-002、WH-003、WH-020                 |
| 多 Thread 分段与按需详情       | WH-002、WH-004、WH-005、WH-018、WH-023 |
| Work History 聚合与分页        | WH-007、WH-008、WH-011、WH-019、WH-024 |
| Multi-Agent 独立路线           | WH-012、WH-013、WH-014                 |
| Round 真实归属和历史兼容       | WH-013、WH-015                         |
| Acceptance 真实 Evidence       | WH-016、WH-017                         |
| Inspector/Drawer               | WH-009、WH-010、WH-022                 |
| 不过度设计/不伪造字段          | WH-005、WH-015、WH-016、WH-017、WH-020 |
| 原 Activity 能力回归           | WH-001、WH-021                         |
| 鉴权和数据保护                 | WH-005、WH-019                         |
...
- 基础命令全部通过。
- WH-001 至 WH-024 必跑项均有真实执行结果和证据。
