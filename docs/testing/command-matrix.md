...
| ---------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 测试计划或测试目录治理                   | `pnpm docs:check`；`pnpm testplan:verify`；`pnpm testplan:validate [path]`；`git diff --check`                                         | 无产品行为变更时不启动运行环境；确认仅保留 `*.testplan.yaml`、每份不超过 20 条 case，且 `docs/testing/README.md` 索引一致。                                       |
| 前端文案、样式、TSX 组件                 | `pnpm --filter @runweave/frontend typecheck`；`pnpm --filter @runweave/frontend lint`                                                  | 当前无 tracked E2E spec；从对应 YAML 选择场景并用 `$toolkit:playwright-cli` 验收真实页面。                                                                        |
| Worktree Project Context                 | `pnpm --filter @runweave/backend typecheck`；`pnpm --filter @runweave/frontend typecheck`                                              | 按 `terminal/worktree-project-context.testplan.yaml` 用 `$toolkit:playwright-cli` 取证；不要引用已删除的历史 E2E。                                                |
...
| Agent Team、outbox、repair 或 checkpoint | `pnpm --filter @runweave/backend typecheck`；`pnpm agent-team:verify-review-checkpoints`                                               | 从 `agent-team/README.md` 选择 lifecycle、execution、recovery 或 completion 权威计划，在真实 tmux pane、outbox 和 sidecar 上验收。                                |
...
| 原生 iOS 连接、认证或终端                | `pnpm ios:doctor`；`pnpm ios:build -- --simulator <UDID>`；对应 YAML 校验                                                              | 按 `app/ios-native-*.testplan.yaml` 在 Simulator / 真机实际操作；SwiftUI 不用 Playwright 验收。                                                                   |
...
| 预合并全量自动化信心                     | `pnpm quality:gate`                                                                                                                    | 该门禁会按变更选择 docs、架构、typecheck、lint 与 `pnpm test:e2e`；当前无 tracked E2E spec，E2E 层会以 `No tests found` 阻塞，不能报告全量通过。                  |
| 纯文档整理或文档保鲜                     | `pnpm docs:check`；`git diff --check`                                                                                                  | 用 `git diff --name-only` 确认只改允许的文档范围；仅在测试计划格式或清单变更时执行对应 `testplan` 校验。                                                          |
