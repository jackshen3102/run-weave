...
| 前端非 UI 逻辑（`*.ts`）    | `pnpm run test:ui`                                                                                                                                    |
| 客户端模式 / URL / 重连策略 | `pnpm --filter ./frontend test:ui -- src/features/client-mode.test.ts src/features/viewer/url.test.ts src/features/terminal/reconnect-policy.test.ts` |
