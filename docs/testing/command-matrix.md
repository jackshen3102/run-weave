...
| 终端链路 / Vim / 预览     | `pnpm --filter ./frontend exec playwright test tests/terminal.spec.ts tests/terminal-vim.spec.ts tests/terminal-preview.spec.ts`，再按 `terminal-state-test-cases.md`、`runbooks/terminal-vim.md`、Preview 对应用例使用 `$playwright-cli` 真实验收 |
...
| Web 日志上报入口          | `pnpm --filter ./frontend exec playwright test tests/terminal-diagnostic-logs.spec.ts`，再按 `docs/quality/ai-diagnostic-logging.md` 使用 `$playwright-cli` 真实验收                                                                               |
