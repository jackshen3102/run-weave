...
2. App Server 从事件投影出轻量 Thread / Agent Session 状态，不保存 thread 详情正文。
3. App Server 提供状态查询接口，让外部可读取当前 thread / session 状态。
...
AgentSessionRef 表示某个 agent 运行实例的当前视图。
...
agentSessionId: codex:thread-state-sync-001
agent: codex
...
3. 查询 ThreadRef 和 AgentSessionRef。

预期：

- ThreadRef `status=running`。
- AgentSessionRef 存在且 `status=running`。
- 两者 `lastEventId` 都推进到最新事件。
- WebSocket `/events/stream` 订阅者收到 live event。
- 如果实现了状态变化事件，订阅者还应收到 thread/session 状态变化事件。
...
3. 查询 ThreadRef、AgentSessionRef 和 `/events`。

预期：

- ThreadRef `status=idle`。
- AgentSessionRef `status=idle`。
...
- 直接删除 ThreadRef 或 AgentSessionRef。
...
4. 查询 ThreadRef 和 AgentSessionRef。
...
- AgentSessionRef `status=completed`。
...
3. 查询 AgentSessionRef。

预期：

- 系统创建可追踪的降级 AgentSessionRef。
- 状态为 `starting`。
...
4. 查询 ThreadRef、AgentSessionRef、`/events`。
...
2. 检查 `projections/latest-threads.json` 和 `projections/latest-agent-sessions.json`。
...
1. 不带 token 查询 thread/session 状态接口。
2. 使用错误 token 查询。
3. 使用正确 token 查询。
...
- live event 中包含可定位的 `threadId`、`agentSessionId`、`status`、`lastEventId`。
- 如果重复事件没有造成状态变化，不应重复推送状态变化事件。

失败判断：

- 只能通过轮询状态接口观察变化。
- WebSocket 推送缺少 thread/session 归属字段。
...
5. 查询 App Server ThreadRef / AgentSessionRef。
...
- 本地同步目录保留两个 thread/session 的 latest 状态。
...
2. 状态查询 API 路径命名，例如 `/threads`、`/threads/:threadId`、`/agent-sessions`。
