...
| ID            | 范围                      | 命令                                                                                                           | 预期                                                      |
| ------------- | ------------------------- | -------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------- |
| DO-STATIC-001 | shared/backend health     | `pnpm --filter @browser-viewer/backend test -- src/server/health.test.ts`                                      | `/health` payload 测试通过                                |
| DO-STATIC-002 | terminal-events heartbeat | `pnpm --filter @browser-viewer/backend test -- src/ws/terminal-events-server.test.ts src/ws/heartbeat.test.ts` | terminal-events 既有消息和 heartbeat 测试通过             |
