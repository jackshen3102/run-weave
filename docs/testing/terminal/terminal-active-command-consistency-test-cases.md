...
| AC-SYS-013 | 绝对路径命令               | 执行 `/bin/sleep 5`                                                                                                                          | API/WS 在运行期间报告 `/bin/sleep` 或规范化后的 `sleep`，结束后为 `null`；UI 展示与 `displayCommand` 实现保持一致，不以括号增强文案作为阻塞条件                           |
...
| AC-SYS-021 | session 退出清空           | 创建显式 `runtimePreference=pty` 的非交互命令 terminal，例如 `sleep 2` 作为启动命令，等待进程退出                                            | session `status="exited"` 后，目标契约为 `activeCommand=null`；不得在列表长期显示 `(sleep)`；默认 tmux 非交互命令退出后会按恢复契约回到 shell，不作为本 Case 的退出态前置 |
...
| 层级             | 入口                                                                                          | 覆盖重点                                                                                       |
| ---------------- | --------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| Web E2E          | 当前仅有 `frontend/tests/smoke.spec.ts`；具体行为使用 Dev Session + `$toolkit:playwright-cli` | UI tab、workspace list、history drawer 与 API activeCommand 一致；输入命令、刷新、结束、Ctrl-C |
