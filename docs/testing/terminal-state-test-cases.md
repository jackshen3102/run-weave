...
| 层级                        | 文件/入口                                                                          | 目标                                                                                              |
| --------------------------- | ---------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| backend service             | `backend/src/terminal/terminal-state-service.test.ts`                              | 穷举状态机映射、优先级、状态覆盖、反向契约                                                        |
| backend routes              | `backend/src/routes/terminal-state.test.ts`、`backend/src/routes/terminal.test.ts` | 验证 hook 鉴权、body 校验、session 校验、API payload、App overview                                |
| backend websocket           | `backend/src/ws/terminal-server.test.ts`                                           | 验证 metadata activeCommand 变化会写入状态，未变化不重复写入                                      |
| CLI                         | `packages/runweave-cli/src/commands/terminal.test.ts`                              | 验证 handoff 输出以后端 TerminalState 为准，interrupt 不写状态                                    |
| Electron hook               | `electron/src/hooks/hook-installer.test.ts`                                        | 验证 Codex hooks 上报到 `/internal/terminal/agent-hook`，body 携带 terminal id 和 token           |
...
1. 先补 `TerminalStateService` 的目标契约测试，尤其是 `command="codex"` + `activeCommand=null/node` 的反向案例。
2. 再补 route 级 hook gate 测试，确保 stale `command="codex"` 不会放宽 `UserPromptSubmit/Stop`。
3. 补 `/state` 禁用 live tail heuristic 的测试，固定 tail/scrollback 不能把任何状态推进到 `agent_running`。
4. 补 App home overview 和 CLI handoff 以 `/state` 为准的测试。
...
pnpm --filter ./electron test -- hook-installer
pnpm --filter ./electron typecheck
```

前端 UI 行为不新增 `src/` 下 Vitest 单测。Web 行为通过 `frontend/tests` E2E 覆盖；App terminal detail 通过 App manual / App Playwright 单独验收。需要自动化页面验证时，使用 Playwright E2E；需要人工页面验收时，按仓库约束通过 `$playwright-cli` 打开和操作页面。
