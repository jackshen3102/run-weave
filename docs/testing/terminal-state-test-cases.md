...
本文档不要求新增单测、Vitest、Node test 或 Playwright E2E spec。涉及浏览器页面、真实终端、Codex TUI 可输入状态的验证，必须通过普通开发服务 + `$toolkit:playwright-cli` 操作真实页面和真实 API 流量完成。
...
4. 用真实浏览器流量验证 Codex TUI ready：普通 dev 后端 + 普通 dev 前端 + `$toolkit:playwright-cli` 登录页面、创建真实 Codex terminal、读取真实 `/api/terminal/session/:id/state`。
...
本仓库不新增或维护单测。TerminalState 这类真实终端状态验收优先使用普通开发服务 + `$toolkit:playwright-cli` 的真实浏览器流量；不要为了本文档新增或执行 Playwright E2E spec。App terminal detail 通过 App manual / App Playwright 单独验收。
