...
本文档不要求新增单测、Vitest、Node test 或 Playwright E2E spec。涉及浏览器页面、真实终端、Codex TUI 可输入状态的验证，必须通过普通开发服务 + `$playwright-cli` 操作真实页面和真实 API 流量完成。
...
- Codex TUI ready detector：`packages/shared/src/terminal-agent-readiness.ts`。
...
- CLI agent start：`packages/runweave-cli/src/commands/terminal-agent.ts` 对 `agent_starting` 的 Codex ready fallback 复用共享 detector。
...
- `/state` 会读取 live scrollback，并只允许通过共享 Codex ready detector 把 `agent_starting/codex` 推进到 `agent_idle/codex`。这不是 running 推断。
- `/state` 不应读取 live tail 推断 `agent_running`。如果当前代码存在 `Working (... esc to interrupt ...)` 这类输出 heuristic，应视为需要删除的实现缺陷，而不是测试契约。
- Codex TUI 当前没有可靠的官方 ready hook；`SessionStart` 只能代表进程启动，不能代表 prompt 可输入。因此 `OpenAI Codex` / model line / `›` prompt 这类 TUI ready 文本只能作为集中封装的 `agent_starting -> agent_idle` fallback，统一维护在 `packages/shared/src/terminal-agent-readiness.ts`。
...
允许的 scrollback 用途仅限 ready fallback：当状态是 `agent_starting/codex`，且 live scrollback 已出现共享 detector 识别的 Codex prompt，状态可推进为 `agent_idle/codex`。
...
| Browser real-traffic        | 普通 `pnpm dev` 或指定普通 dev 端口 + `$playwright-cli`                                         | 真实登录、真实页面、真实 terminal session、真实 `/api/terminal/session/:id/state`；不跑 E2E spec                  |
| App manual / App Playwright | App dev/simulator + `$playwright-cli` 或手工验收                                                | 覆盖 `app/src/pages/AppTerminalPage.tsx` 的 Stop、composer、terminal-events 状态刷新；不要误归到 `frontend/tests` |
...
| TS-SVC-002 | 无 store；session running，`command="zsh"`，`activeCommand="codex"`，scrollback 无 Codex ready prompt | `getCurrent()`                                                         | `agent_starting/codex`                                 | 应通过       |
| TS-SVC-003 | 无 store；session running，`activeCommand="/usr/local/bin/codex"`，scrollback 无 Codex ready prompt   | `getCurrent()`                                                         | `agent_starting/codex`                                 | 应通过       |
...
| TS-SVC-017 | 当前为 `agent_starting`；live scrollback 含 `OpenAI Codex` / model line / `›` prompt                  | `getCurrent()`                                                         | `agent_idle/codex`，并写入 store                       | 应通过       |
...
| TS-API-002 | session running，`activeCommand="codex"`，无 store，live scrollback 尚无 Codex ready prompt        | `GET /state`                                | 200，`agent_starting/codex`                                         |
...
| TS-API-004 | session running，store 为 `agent_starting`，live scrollback 命中 Codex ready prompt                | `GET /state`                                | 200，推进为 `agent_idle/codex`；使用 live scrollback ready fallback |
...
| TS-WS-007 | tmux metadata reader 返回 `activeCommand="/path/to/codex"`，尚无 ready prompt | sync metadata                           | 状态为 `agent_starting/codex`                                                      |
...
| TS-HOME-002 | running session，`activeCommand="codex"`，无 running hook，尚无 ready prompt    | `GET /home/overview`         | `displayStatus="agent-starting"`，`terminalState=agent_starting/codex`                                            |
| TS-HOME-008 | running session，`activeCommand="codex"`，live scrollback 有 Codex ready prompt | `GET /home/overview`         | `displayStatus="agent-idle"`，`terminalState=agent_idle/codex`                                                    |
...
| TS-CLI-008 | `rw terminal send --agent codex` 启动 Codex 后，后端 `/state` 暂为 `agent_starting/codex`，scrollback 已有 ready prompt | `rw terminal send <id> --agent codex --text ... --enter --json` | CLI 使用共享 Codex ready detector 判断可发送，不因 hook 缺失等待超时                                         |
...
3. 确认 `/state` 只把 live scrollback 用作 Codex ready fallback，不让 tail/scrollback 把任何状态推进到 `agent_running`。
4. 用真实浏览器流量验证 Codex TUI ready：普通 dev 后端 + 普通 dev 前端 + `$playwright-cli` 登录页面、创建真实 Codex terminal、读取真实 `/api/terminal/session/:id/state`。
...
1. 启动普通开发后端和前端，端口可临时指定，但不要设置 `RUNWEAVE_E2E_TEST_ROUTES=true`。
2. 用 `$playwright-cli` 打开前端登录页，使用真实登录表单登录。
3. 通过页面真实 API 创建 terminal session，命令使用本机真实 `codex`，例如 `command="/opt/homebrew/bin/codex"`，`runtimePreference="tmux"`。
4. 打开 `/terminal/<terminalSessionId>`，等待页面出现 `OpenAI Codex`、model line 和 `›` prompt。
5. 在同一页面上下文内 `fetch("/api/terminal/session/<id>/state")`。
6. 预期：Codex TUI ready 前为 `agent_starting/codex`；ready prompt 出现后为 `agent_idle/codex`；提交 prompt 后只有 hook `UserPromptSubmit` 能进入 `agent_running/codex`。
...
本仓库不新增或维护单测。TerminalState 这类真实终端状态验收优先使用普通开发服务 + `$playwright-cli` 的真实浏览器流量；不要为了本文档新增或执行 Playwright E2E spec。App terminal detail 通过 App manual / App Playwright 单独验收。
