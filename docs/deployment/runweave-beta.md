...
Beta Pool 行为验收按能力边界拆成两个可独立执行的合同，避免把相同恢复断言在一个“全场景”文件中重复维护：

- [控制面与恢复验收](../testing/platform/beta-pool-storage-migration.testplan.yaml)：保留历史文件入口，但内容只覆盖当前 canonical 初始化、只读投影、lease 分配、异常识别、安全恢复、并发竞争、崩溃收敛和诊断发布，不执行旧数据迁移 Case。
- [长期运行与生产回归验收](../testing/platform/beta-pool-runtime-regressions.testplan.yaml)：十轮 start/stop、十轮 update、owner 交接、真实 CDP、磁盘、Stable/shared、跨 HOME 所有权和 App 更新崩溃恢复。

两个计划都只使用当前固定五槽与 canonical 行为，不构造或迁移 legacy 控制面旧数据，也不执行 migration journal、tombstone、旧备份或降级测试。需要验证 `dev-session --profile beta` 时，以这两个合同和 `dev:status` / `dev:open` 返回的 slot、lease、manifest、CDP 身份为准。
...
| 历史合同                       | 当前归属                     | 处理                                                              |
| ------------------------------ | ---------------------------- | ----------------------------------------------------------------- |
| BSP-001 首次固定槽位启动       | BETA-002、BPR-001            | 按 ownership HOME 与全局 App/pointer 的当前边界更新               |
| BSP-002 十轮 start/stop        | BPR-001                      | 原次数、安装目标上限和 Stable 基线保留                            |
| BSP-003 十轮 update 有界       | BPR-002                      | 原 release、rollback、日志和字节断言保留                          |
| BSP-004 五槽并发容量           | BETA-016                     | 由当前 hard-link lease 与 canonical projection 覆盖               |
| BSP-005 reset 前不可交接       | BPR-003                      | 独立于行为态清理取证                                              |
| BSP-006 新 owner 行为态隔离    | BPR-004                      | 保留真实 desktop/terminal-browser 证据                            |
| BSP-007 stale/orphan 身份恢复  | BETA-005、BETA-006、BETA-008 | 按当前 recovery claim、receipt 和 blocker 拆细                    |
| BSP-008 active owner 不被清理  | BETA-005、BETA-009、BPR-007  | recovery 与 shared/Stable 边界分别取证                            |
| BSP-009 status/open/CDP 身份   | BETA-015、BPR-005            | projection 只读与真实 CDP 分开取证                                |
| BSP-010 磁盘门禁               | BPR-006                      | 保留清理后重算与完整字节诊断                                      |
| BSP-011 显式 slot 边界         | BETA-013                     | 更新为当前 `--profile beta --instance pool-0N` 入口               |
| BSP-012 dry-run 只读           | BETA-001                     | 更新为显式 beta profile，不依赖 changed-files 推断                |
| BSP-013 shared/Stable 不被清理 | BPR-007                      | 按当前 planner ownership 取真实基线                               |
| BSP-014 损坏/未知 lease        | BETA-008、BETA-010           | 按当前 quarantine 和无 force 后门覆盖                             |
| BSP-015 start failure release  | BETA-011、BPR-008            | release 崩溃窗口与前置失败 lease 收敛分开取证                     |
| BSP-016 legacy cleanup/purge   | 不执行                       | 按当前要求显式退役，不构造旧控制面数据或迁移现场                  |
| BSP-017 warm slot 重试         | BETA-002                     | 与正常停止/再次启动合同合并，保留同 HOME、同 slot 和新 nonce 边界 |
