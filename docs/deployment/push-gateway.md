...
手机在连接管理中可手动启用“低电量提醒”。已登录的连接会自动注册定时任务提醒；授予系统通知权限后，手机完成对应订阅确认。
如果之前绑定了本机试运行网关，先在旧网关可达时关闭提醒，完成撤销，再切换 URL 并重新启用；
无需迁移试运行数据。完成后停止旧试运行网关的自动启动，避免误用旧地址。

Backend 将订阅同步状态绑定到网关 HTTPS origin。切换 origin 后，或升级遇到未记录网关的旧订阅，
会持久化新的订阅版本并重新注册；旧确认状态和撤销凭据不跨网关沿用。注册失败保持 pending，
后续重试复用该版本；注册成功后仍需手机回到前台同步并确认，才能恢复投递。
相同 origin 的正常重启不重复注册。此保护不撤销旧网关上的订阅，不能替代上面的旧网关撤销流程；
相同 origin 下替换或清空网关数据库也不属于地址变更，仍须按部署恢复流程处理。
