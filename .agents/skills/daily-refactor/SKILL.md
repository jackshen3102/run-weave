---
name: daily-refactor
description: 仅当用户明确要求使用 daily-refactor skill 时使用；用于每天按 main 分支上一天 00:00 到当前的业务代码提交范围执行重构、验证、提交、push 并创建 GitHub PR。
---
...
每天唤起后直接完成一次端到端重构交付：基于 `main` 分支上一天 00:00 到当前的 commit 范围，分析业务代码变更，做重构前基线验证，重构业务代码，做重构后回归验证，提交、push，并创建 GitHub PR。
...
1. 看当前分支、远程、工作区状态，以及 `main` 分支默认时间窗口内的 commit。
